...
class SalesInvoice(SellingController):
    ...
    def validate(self):
        ...
        is_deferred_invoice = any(d.get("enable_deferred_revenue")
            ...
            if d.is_fixed_asset and d.meta.get_field("asset") and d.asset:
                ...
                if self.doctype == "Sales Invoice" and self.docstatus == 1:
                    if self.update_stock:
                        frappe.throw(
                            _("'Update Stock' cannot be checked for fixed asset sale"))
            ...
            if cost_center_company != self.company:
                frappe.throw(
                    _("Row #{0}: Cost Center {1} does not belong to company {2}").format(
                        frappe.bold(item.idx), frappe.bold(
                            item.cost_center), frappe.bold(self.company)
                    )
                )
    ...
    def validate_income_account(self):
        for item in self.get("items"):
            validate_account_head(
                item.idx, item.income_account, self.company, "Income")
        ...
        if self.doctype == "Sales Invoice" and self.is_consolidated:
            ...
            if pos_closing_entry and pos_closing_entry[0]:
                msg = _("To cancel a {} you need to cancel the POS Closing Entry {}.").format(
                    ...
                    get_link_to_form("POS Closing Entry",
                                     pos_closing_entry[0]),
                )
    ...
    def check_credit_limit(self):
        ...
        bypass_credit_limit_check_at_sales_order = frappe.db.get_value(
            ...
            filters={"parent": self.customer,
                     "parenttype": "Customer", "company": self.company},
    ...
    def set_account_for_mode_of_payment(self):
        for payment in self.payments:
            payment.account = get_bank_cash_account(
                payment.mode_of_payment, self.company).get("account")

    def validate_time_sheets_are_submitted(self):
        for data in self.timesheets:
            if data.time_sheet:
                status = frappe.db.get_value(
                    "Timesheet", data.time_sheet, "status")
                if status not in ["Submitted", "Payslip"]:
                    frappe.throw(
                        _("Timesheet {0} is already completed or cancelled").format(data.time_sheet))
    ...
    def set_pos_fields(self, for_validate=False):
        ...
        if pos:
            ...
            if self.customer:
                ...
                selling_price_list = (
                    customer_price_list or customer_group_price_list or pos.get(
                        "selling_price_list")
                )
            else:
    ...
    def validate_debit_to_acc(self):
        if not self.debit_to:
            self.debit_to = get_party_account(
                "Customer", self.customer, self.company)
            if not self.debit_to:
                self.raise_missing_debit_credit_account_error(
                    "Customer", self.customer)
    ...
    def clear_unallocated_mode_of_payments(self):
        self.set("payments", self.get("payments", {
    ...
    def force_set_against_income_account(self):
        ...
        frappe.db.set_value(self.doctype, self.name,
                            "against_income_account", self.against_income_account)

    def add_remarks(self):
        if not self.remarks:
            if self.po_no:
                self.remarks = _(
                    "Against Customer Order {0}").format(self.po_no)
                if self.po_date:
                    self.remarks += " " + \
                        _("dated {0}").format(formatdate(self.po_date))
    ...
    def validate_proj_cust(self):
        ...
        if self.project and self.customer:
            ...
            if not res:
                throw(_("Customer {0} does not belong to project {1}").format(
    ...
    def validate_delivery_note(self):
        for d in self.get("items"):
            if d.delivery_note:
                msgprint(
                    _("Stock cannot be updated against Delivery Note {0}").format(
                        d.delivery_note),
                    raise_exception=1,
                )
        ...
        for item in self.items:
            if item.sales_order:
                if frappe.db.get_value("Sales Order Item", item.so_detail, "delivered_by_supplier"):
                    frappe.throw(
                        _("Could not update stock, invoice contains drop shipping item."))
    ...
    def get_warehouse(self):
        ...
        if not warehouse:
            ...
            if global_pos_profile:
                ...
                msgprint(_("POS Profile required to make POS Entry"),
    ...
    def make_customer_gl_entry(self, gl_entries):
        ...
        grand_total = (
            self.rounded_total if (
                self.rounding_adjustment and self.rounded_total) else self.grand_total
        )
    ...
    def make_tax_gl_entries(self, gl_entries):
        ...
        for tax in self.get("taxes"):
            amount, base_amount = self.get_tax_amounts(
                tax, enable_discount_accounting)
                    ...
                    self.get_gl_dict(
                        {
                            ...
                            "credit_in_account_currency": (
                                flt(base_amount, tax.precision(
                                    "base_tax_amount_after_discount_amount"))
                                if account_currency == self.company_currency
                                else flt(amount, tax.precision("tax_amount_after_discount_amount"))
                            ),
                            "credit_in_transaction_currency": flt(
                                amount, tax.precision(
                    ...
                    if (self.docstatus == 2 and not self.is_return) or (
                        ...
                        if asset.calculate_depreciation:
                            ...
                            notes = _(
                                ...
                                get_link_to_form(
                                    self.doctype, self.get("name")),
                            )
                            reset_depreciation_schedule(
                                asset, self.posting_date, notes)
                            asset.reload()
                        ...
                        self.get_gl_dict(
                            {
                                ...
                                "credit_in_account_currency": (
                                    flt(base_amount, item.precision(
                                        "base_net_amount"))
                                    if account_currency == self.company_currency
    ...
    def make_gle_for_change_amount(self, gl_entries):
        if self.change_amount:
            if self.account_for_change_amount:
                ...
                frappe.throw(_("Select change amount account"),
    ...
    def make_write_off_gl_entry(self, gl_entries):
        ...
        if (
            ...
            write_off_account_currency = get_account_currency(
                ...
                self.get_gl_dict(
                    {
                        ...
                        "credit_in_account_currency": (
                            flt(self.base_write_off_amount, self.precision(
                                "base_write_off_amount"))
                            if self.party_account_currency == self.company_currency
                            else flt(self.write_off_amount, self.precision("write_off_amount"))
                        ),
                        "credit_in_transaction_currency": flt(
                            self.write_off_amount, self.precision(
                                "write_off_amount")
                        ),
                        "against_voucher": self.return_against if cint(self.is_return) else self.name,
                        "against_voucher_type": self.doctype,
                ...
                self.get_gl_dict(
                    {
                        ...
                        "credit_in_account_currency": flt(
                            self.rounding_adjustment, self.precision(
                                "rounding_adjustment")
                        ),
                        "credit_in_transaction_currency": flt(
                            self.rounding_adjustment, self.precision(
                        ...
                        "credit": flt(
                            self.base_rounding_adjustment, self.precision(
    ...
    def update_project(self):
        unique_projects = list(
            set([d.project for d in self.get("items") if d.project]))
    ...
    def apply_loyalty_points(self):
        ...
        for lp_entry in loyalty_point_entries:
            if lp_entry.invoice_type != self.doctype or lp_entry.invoice == self.name:
                ...
                continue
            available_points = lp_entry.loyalty_points - \
                flt(redemption_details.get(lp_entry.name))
...
def validate_inter_company_party(doctype, party, company, inter_company_reference):
    ...
    if inter_company_reference:
        ...
        ref_party = doc.supplier if doctype in [
            "Sales Invoice", "Sales Order"] else doc.customer
...
def make_delivery_note(source_name, target_doc=None):
    ...
    def update_item(source_doc, target_doc, source_parent):
        ...
        target_doc.stock_qty = target_doc.qty * \
            flt(source_doc.conversion_factor)
...
def validate_inter_company_transaction(doc, doctype):
    ...
    if not party:
        partytype = "Supplier" if doctype in [
            "Sales Invoice", "Sales Order"] else "Customer"
        frappe.throw(
    ...
    def update_details(source_doc, target_doc, source_parent):
        ...
        if target_doc.doctype in ["Purchase Invoice", "Purchase Order"]:
            ...
            if source_doc.company_address and _validate_address_link(
                    source_doc.company_address, "Supplier", details.get(
                        "party")
            ):
                update_address(target_doc, "supplier_address",
                               "address_display", source_doc.company_address)
            if source_doc.dispatch_address_name and _validate_address_link(
                    source_doc.dispatch_address_name, "Company", details.get(
                        "company")
            ):
                update_address(
                    target_doc,
            ...
            if source_doc.shipping_address_name and _validate_address_link(
                    source_doc.shipping_address_name, "Company", details.get(
                        "company")
            ):
                update_address(
                    target_doc,
            ...
            if source_doc.customer_address and _validate_address_link(
                    source_doc.customer_address, "Company", details.get(
                        "company")
            ):
                update_address(
            ...
            if source_doc.supplier_address and _validate_address_link(
                    source_doc.supplier_address, "Company", details.get(
                        "company")
            ):
                update_address(
            ...
            if source_doc.shipping_address and _validate_address_link(
                    source_doc.shipping_address, "Customer", details.get(
                        "party")
            ):
                update_address(
            ...
            if source_doc.shipping_address and _validate_address_link(
                ...
                update_address(target_doc, "customer_address",
                               "address_display", source_doc.shipping_address)
...
def set_purchase_references(doc):
    ...
    if doc.is_internal_transfer():
        if doc.doctype == "Purchase Receipt":
            so_item_map = get_delivery_note_details(
            ...
            if so_item_map:
                ...
                update_pr_items(doc, so_item_map, pd_item_map,
                                parent_child_map, warehouse_map)
...
def update_pi_items(
    ...
    for item in doc.get("items"):
        item.set(detail_field, purchase_item_map.get(
            sales_item_map.get(item.sales_invoice_item)))
        item.set(parent_field, parent_child_map.get(
            sales_item_map.get(item.sales_invoice_item)))
        if doc.update_stock:
...
def update_pr_items(doc, sales_item_map, purchase_item_map, parent_child_map, warehouse_map):
    for item in doc.get("items"):
        item.warehouse = warehouse_map.get(
            sales_item_map.get(item.delivery_note_item))
        if not item.warehouse and item.get("purchase_order") and item.get("purchase_order_item"):
            item.warehouse = frappe.db.get_value(
                "Purchase Order Item", item.purchase_order_item, "warehouse")
...
def update_multi_mode_option(doc, pos_profile):
    ...
    mode_of_payments_info = get_mode_of_payments_info(
        mode_of_payments, doc.company)

    for row in pos_profile.get("payments"):
        payment_mode = mode_of_payments_info.get(row.mode_of_payment)
        if not payment_mode:
            invalid_modes.append(get_link_to_form(
...
# ============================================================
#
# Strategy: Direct field + GL + PLE update — no cancel/resubmit
#
# For a Sales Invoice the GL structure is always:
#   DEBIT  : Debtor (receivable) account  = grand_total
#   CREDIT : Income / Sales account(s)    = net_total per item
#   CREDIT : CGST Output account          = cgst_amount
#   CREDIT : SGST Output account          = sgst_amount
#   CREDIT : Freight income account       = freight (if any)
#
# We directly update each GL entry's debit/credit by the delta.
# No new entries → no imbalance risk.
# ============================================================


@frappe.whitelist()
def update_sales_invoice(si_name, posting_date, freight, item_rates, reason, add_freight_item=0):
    ...
    if si.is_return:
        frappe.throw("Cannot edit a Credit Note.")

    old_grand = flt(si.grand_total, 2)
    old_net_total = flt(si.net_total, 2)
    ...
    # ── Block if fully paid ───────────────────────────────────
    if old_outstanding <= 0 and old_paid > 0:
        frappe.throw(
            f"Invoice <b>{si_name}</b> is fully paid "
            f"(Paid: ₹{old_paid:.2f}). Editing is not allowed.",
            title="Editing Blocked — Invoice Fully Paid"
        )

    add_freight_item = flt(add_freight_item)

    # ── Apply item rate changes in-memory ─────────────────────
    ...
    # ── Add FREIGHT as new item row if fresh freight entered ──
    if add_freight_item > 0:
        # Ensure FREIGHT item exists
        if not frappe.db.exists("Item", "FREIGHT"):
            _create_freight_item()

        # Get income account and cost center from first item
        first = si.items[0] if si.items else frappe._dict()
        income_account = frappe.db.get_value(
            "Item Default",
            {"parent": "FREIGHT", "company": si.company},
            "income_account"
        ) or first.get("income_account", "")
        ...
        si.append("items", {
            ...
            "qty":            1,
            "rate":           add_freight_item,
            "amount":         add_freight_item,
            "uom":            "Nos",
            "income_account": income_account,
            "cost_center":    first.get("cost_center", si.cost_center or ""),
        })
    ...
    new_freight = flt(si.freight or 0, 2) if hasattr(si, "freight") else 0

    # Individual deltas
    grand_diff = flt(new_grand - old_grand, 2)
    net_diff = flt(new_net_total - old_net_total, 2)
    freight_diff = flt(new_freight - old_freight, 2)

    # Tax deltas per account
    tax_deltas = {}   # {account_head: delta}
    for tax in si.taxes:
        acc = tax.account_head
        # old amount from DB before recalc (si was reloaded, taxes recalculated)
        # We need old tax amounts — fetch from DB
        old_tax = frappe.db.get_value(
            "Sales Taxes and Charges", tax.name, "tax_amount"
        ) or 0
        delta = flt(tax.tax_amount, 2) - flt(old_tax, 2)
        if acc not in tax_deltas:
            tax_deltas[acc] = 0
        tax_deltas[acc] = flt(tax_deltas[acc] + delta, 2)

    # ── Step 1: Save updated item amounts ────────────────────
    # For new FREIGHT row (no name yet) — insert it first
    for item in si.items:
        if not item.name:
            # New row appended in memory — insert into DB
            item.parent = si_name
            item.parenttype = "Sales Invoice"
            item.parentfield = "items"
            item.idx = len(si.items)
            ...
            item_doc.insert()
            item.name = item_doc.name
        else:
            frappe.db.set_value("Sales Invoice Item", item.name,
                                {"rate": item.rate, "amount": item.amount},
                                update_modified=False)

    # ── Step 2: Save updated tax amounts ─────────────────────
    for tax in si.taxes:
        frappe.db.set_value("Sales Taxes and Charges", tax.name, {
    ...
    # ── Step 3: Save SI header ────────────────────────────────
    new_outstanding = max(0, flt(new_grand - old_paid, 2))
    ...
    header = {
        ...
        "paid_amount":             old_paid,      # ← never changed
        "modified":                now(),
        "modified_by":             frappe.session.user,
    }
    if hasattr(si, "freight"):
        header["freight"] = new_freight
    if str(posting_date) != str(old_date):
        header["posting_date"] = posting_date

    frappe.db.set_value("Sales Invoice", si_name, header,
                        update_modified=False)
    frappe.db.commit()

    # ── Step 4: Update existing GL Entries directly ───────────
    # Fetch all active GL entries for this SI
    ...
    # Identify the debtor (receivable) row — it has party=customer
    # and debit > 0 (money owed by customer)
    debtor_account = si.debit_to

    for gl in gl_rows:
        new_debit = flt(gl.debit, 2)
        new_credit = flt(gl.credit, 2)
        new_debit_acc = flt(gl.debit_in_account_currency, 2)
        new_credit_acc = flt(gl.credit_in_account_currency, 2)

        if gl.account == debtor_account and gl.debit > 0:
            # Receivable row — increases by grand_diff
            new_debit = flt(gl.debit + grand_diff, 2)
            new_debit_acc = flt(gl.debit_in_account_currency + grand_diff, 2)

        elif gl.account in tax_deltas and gl.credit > 0:
            # Tax account row (CGST/SGST/IGST) — update by tax delta
            delta = tax_deltas[gl.account]
            new_credit = flt(gl.credit + delta, 2)
            new_credit_acc = flt(gl.credit_in_account_currency + delta, 2)

        elif gl.credit > 0 and gl.account != debtor_account:
            # Income / freight row — check if it's freight
            if freight_diff != 0 and _is_freight_account(gl.account, si):
                new_credit = flt(gl.credit + freight_diff, 2)
                new_credit_acc = flt(
                    gl.credit_in_account_currency + freight_diff, 2)
            elif gl.account not in tax_deltas:
                # Regular income account — update by net_diff minus freight_diff
                income_diff = flt(net_diff - freight_diff, 2)
                new_credit = flt(gl.credit + income_diff, 2)
                new_credit_acc = flt(
                    gl.credit_in_account_currency + income_diff, 2)

        frappe.db.sql("""
            UPDATE `tabGL Entry`
            SET    debit                       = %(d)s,
                   credit                      = %(c)s,
                   debit_in_account_currency   = %(da)s,
                   credit_in_account_currency  = %(ca)s,
                   modified                    = %(now)s,
                   modified_by                 = %(user)s
            WHERE  name = %(name)s
        """, {
            "d":    new_debit,     "c":    new_credit,
            "da":   new_debit_acc, "ca":   new_credit_acc,
            "now":  now(),
    ...
    # ── Step 5: Update Payment Ledger Entry — set absolute amount ──
    # Use new_grand as absolute value (not delta) to avoid drift.
    # Filter on against_voucher_no which is how ERPNext stores SI ref in PLE.
    frappe.db.sql("""
        UPDATE `tabPayment Ledger Entry`
        SET    amount                     = %(grand)s,
               amount_in_account_currency = %(grand)s,
               modified                   = %(now)s,
               modified_by                = %(user)s
        WHERE  against_voucher_no = %(si)s
          AND  voucher_type       = 'Sales Invoice'
          AND  delinked           = 0
    """, {
        "grand": new_grand, "si": si_name,
        "now": now(), "user": frappe.session.user
    })
    frappe.db.commit()

    # ── Step 6: Update Payment Entry Reference ────────────────
    frappe.db.sql("""
        UPDATE `tabPayment Entry Reference`
        SET    outstanding_amount = %(os)s,
               modified          = %(now)s
        WHERE  reference_doctype = 'Sales Invoice'
          AND  reference_name    = %(si)s
          AND  docstatus         = 1
    """, {"os": new_outstanding, "si": si_name, "now": now()})

    frappe.db.commit()

    # ── Step 7: Audit comment ─────────────────────────────────
    lines = [f"<b>Quick Edit</b> by {frappe.session.user}"]
    if str(posting_date) != str(old_date):
        lines.append(f"Posting Date: {old_date} → {posting_date}")
    if abs(new_freight - old_freight) > 0.001:
        lines.append(f"Freight: ₹{old_freight:.2f} → ₹{new_freight:.2f}")
    if item_rates:
    ...
    lines.append(f"Difference: ₹{grand_diff:.2f}")
...
def _create_freight_item():
    """Auto-create FREIGHT item if it doesn't exist."""
...
def _is_freight_account(account, si):
    """Check if a GL account is the freight account for this SI."""
    # Check if the account is linked to freight charges in the SI taxes
    for tax in si.taxes:
        if tax.account_head == account and (
            "freight" in (tax.description or "").lower() or
            "freight" in account.lower()
        ):
            return True
    return False
