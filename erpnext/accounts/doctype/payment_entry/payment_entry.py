...
class PaymentEntry(AccountsController):
    ...
    def on_update_after_submit(self):
        ...
        self.needs_repost = self.check_if_fields_updated(
            fields_to_check=[], child_tables={
                "references": [], "taxes": [], "deductions": []}
        )
    ...
    def set_liability_account(self):
        ...
        account_type = frappe.get_value(
            "Account", {"name": self.party_account,
                        "company": self.company}, "account_type"
        )
        ...
        if self.references:
            ...
            reference_types = set(
                [x.reference_doctype for x in self.references])
        ...
        if not liability_account:
            throw(
                _("Please set default {0} in Company {1}").format(
                    frappe.bold(frappe.get_meta("Company").get_label(
                        fieldname)), frappe.bold(self.company)
                )
            )
    ...
    def validate_duplicate_entry(self):
        ...
        for d in self.get("references"):
            key = (d.reference_doctype, d.reference_name,
                   d.payment_term, d.payment_request)
    ...
    def validate_payment_type_with_outstanding(self):
        total_outstanding = sum(
            d.allocated_amount for d in self.get("references"))
    ...
    def validate_allocated_amount_with_latest_data(self):
        ...
        uniq_vouchers = {(x.reference_doctype, x.reference_name)
                         for x in self.references}
        vouchers = [frappe._dict(
            {"voucher_type": x[0], "voucher_no": x[1]}) for x in uniq_vouchers]
        ...
        for d in latest_references:
            d = frappe._dict(d)
            latest_lookup.setdefault((d.voucher_type, d.voucher_no), frappe._dict())[
                d.payment_term] = d

        for idx, d in enumerate(self.get("references"), start=1):
            latest = latest_lookup.get(
                (d.reference_doctype, d.reference_name)) or frappe._dict()
    ...
    def set_missing_values(self):
        if self.payment_type == "Internal Transfer":
            ...
            _party_name = "title" if self.party_type == "Shareholder" else self.party_type.lower() + \
                "_name"
        ...
        if self.party:
            if self.party_type == "Employee":
                ...
                self.contact_person = get_default_contact(
        ...
        if self.paid_to and (
            ...
            acc = get_account_details(
                self.paid_to, self.posting_date, self.cost_center)
    ...
    def set_missing_ref_details(
        ...
        for d in self.get("references"):
            ...
            if update_ref_details_only_for and (
                    (d.reference_doctype,
                     d.reference_name) not in update_ref_details_only_for
            ):
                continue
    ...
    def set_source_exchange_rate(self, ref_doc=None):
        if self.paid_from:
            if self.paid_from_account_currency == self.company_currency:
                ...
                self.source_exchange_rate = ref_doc.get(
                    "exchange_rate") or ref_doc.get("conversion_rate")
    ...
    def set_target_exchange_rate(self, ref_doc=None):
        if self.paid_from_account_currency == self.paid_to_account_currency:
            ...
            if ref_doc and self.paid_to_account_currency == ref_doc.currency:
                self.target_exchange_rate = ref_doc.get(
                    "exchange_rate") or ref_doc.get("conversion_rate")
                    ...
                    if self.party != ref_doc.get(scrub(self.party_type)):
                        frappe.throw(
                            _("{0} {1} is not associated with {2} {3}").format(
                                _(d.reference_doctype), d.reference_name, _(
                                    self.party_type), self.party
                            )
                        )
                else:
        ...
        for d in self.get("references"):
            ...
            if d.reference_doctype in ("Sales Invoice", "Purchase Invoice"):
                ...
                if outstanding_amount <= 0 and not is_return:
                    no_oustanding_refs.setdefault(
                        d.reference_doctype, []).append(d)
        ...
        for reference_doctype, references in no_oustanding_refs.items():
            frappe.msgprint(
                _(
                    ...
                    frappe.bold(
                        comma_and([d.reference_name for d in references])),
                    _(reference_doctype),
                )
                + "<br><br>"
    ...
    def update_payment_schedule(self, cancel=0):
        ...
        for ref in self.get("references"):
            ...
            if not invoice_paid_amount_map.get(key):
                ...
                for term in payment_schedule:
                    invoice_key = (term.payment_term,
                                   ref.reference_name, ref.reference_doctype)
        ...
        for idx, (key, allocated_amount) in enumerate(invoice_payment_amount_map.items(), 1):
            if not invoice_paid_amount_map.get(key):
                frappe.throw(
                    _("Payment term {0} not used in {1}").format(key[0], key[1]))

            allocated_amount = self.get_allocated_amount_in_transaction_currency(
                allocated_amount, key[2], key[1]
            )

            outstanding = flt(invoice_paid_amount_map.get(
                key, {}).get("outstanding"))
            discounted_amt = flt(invoice_paid_amount_map.get(
                key, {}).get("discounted_amt"))

            conversion_rate = frappe.db.get_value(
                key[2], {"name": key[1]}, "conversion_rate")
            base_paid_amount_precision = get_field_precision(
                frappe.get_meta("Payment Schedule").get_field(
                    "base_paid_amount")
            )
            ...
            base_paid_amount = flt(
                ...
                conversion_rate, base_paid_amount_precision
            )
            base_outstanding = flt(
                allocated_amount * conversion_rate, base_outstanding_precision)
    ...
    def set_tax_withholding(self):
        ...
        tax_withholding_details = get_party_tax_withholding_details(
            args, self.tax_withholding_category)

        if not tax_withholding_details:
            return

        tax_withholding_details.update(
            {"cost_center": self.cost_center or erpnext.get_default_cost_center(
                self.company)}
        )
        ...
        for d in self.taxes:
            if d.account_head == tax_withholding_details.get("account_head"):
                ...
                if d.included_in_paid_amount:
                    tax_withholding_details.update(
                        {"included_in_paid_amount": d.included_in_paid_amount})
    ...
    def calculate_tax_withholding_net_total(self):
        ...
        for d in self.references:
            ...
            net_taxable_outstanding = max(
                0, d.outstanding_amount -
                (d.total_amount - tax_withholding_net_total)
            )
    ...
    def get_order_wise_tax_withholding_net_total(self):
        ...
        docnames = [
            d.reference_name for d in self.references if d.reference_doctype == doctype]
    ...
    def validate_received_amount(self):
        if self.paid_from_account_currency == self.paid_to_account_currency:
            if self.paid_amount < self.received_amount:
                frappe.throw(
                    _("Received Amount cannot be greater than Paid Amount"))
    ...
    def set_amounts_after_tax(self):
        ...
        for tax in self.get("taxes"):
            ...
            base_amount = -1 * \
                tax.base_tax_amount if tax.add_deduct_tax == "Deduct" else tax.base_tax_amount
        ...
        self.paid_amount_after_tax = flt(
            ...
            flt(applicable_tax), self.precision("paid_amount_after_tax")
        )
        ...
        self.received_amount_after_tax = flt(
            flt(self.received_amount) +
            flt(applicable_tax), self.precision("paid_amount_after_tax")
        )
        self.base_received_amount_after_tax = flt(
            flt(self.received_amount_after_tax) *
            flt(self.target_exchange_rate),
            self.precision("base_paid_amount_after_tax"),
        )
    ...
    def set_amounts_in_company_currency(self):
        ...
        if self.paid_amount:
            self.base_paid_amount = flt(
                flt(self.paid_amount) *
                flt(self.source_exchange_rate), self.precision(
                    "base_paid_amount")
            )
    ...
    def calculate_base_allocated_amount_for_reference(self, d) -> float:
        ...
        if d.reference_doctype in get_advance_payment_doctypes():
            ...
            base_allocated_amount += flt(
                ...
                flt(exchange_rate), self.precision("base_paid_amount")
            )
        else:
            ...
            allocated_amount_in_ref_exchange_rate = flt(
                flt(d.allocated_amount) *
                flt(d.exchange_rate), self.precision("base_paid_amount")
            )
            d.exchange_gain_loss = base_allocated_amount - \
    ...
    def set_total_allocated_amount(self):
        ...
        for d in self.get("references"):
            if d.allocated_amount:
                ...
                base_total_allocated_amount += self.calculate_base_allocated_amount_for_reference(
                    d)
    ...
    def set_exchange_gain_loss(self):
        ...
        exchange_gain_loss_rows = [row for row in self.get(
            "deductions") if row.is_exchange_gain_loss]
        exchange_gain_loss_row = exchange_gain_loss_rows.pop(
            0) if exchange_gain_loss_rows else None
    ...
    def set_difference_amount(self):
        ...
        base_party_amount = flt(
            self.base_total_allocated_amount) + flt(base_unallocated_amount)
        included_taxes = self.get_included_taxes()

        if self.payment_type == "Receive":
            self.difference_amount = base_party_amount - \
                self.base_received_amount + included_taxes
        elif self.payment_type == "Pay":
            self.difference_amount = self.base_paid_amount - \
                base_party_amount - included_taxes
        else:
            self.difference_amount = self.base_paid_amount - \
                flt(self.base_received_amount) - included_taxes

        total_deductions = sum(flt(d.amount) for d in self.get("deductions"))

        self.difference_amount = flt(
            self.difference_amount -
            total_deductions, self.precision("difference_amount")
        )
    ...
    def clear_unallocated_reference_document_rows(self):
        self.set("references", self.get("references", {
                 "allocated_amount": ["not in", [0, None, ""]]}))
        frappe.db.sql(
    ...
    def validate_transaction_reference(self):
        ...
        bank_account_type = frappe.get_cached_value(
            "Account", bank_account, "account_type")
    ...
    def add_party_gl_entries(self, gl_entries):
        ...
        for d in self.get("references"):
            ...
            if (
                    d.reference_doctype in [
                        "Sales Invoice", "Purchase Invoice"]
                    and d.allocated_amount < 0
                    and (
                        (party_account_type ==
                         "Receivable" and self.payment_type == "Pay")
            ...
            if self.book_advance_payments_in_separate_party_account:
                if d.reference_doctype in advance_payment_doctypes:
                    ...
                    gle.update({"against_voucher_type": self.doctype,
    ...
    def add_advance_gl_entries(self, gl_entries: list, entry: object | dict | None):
        ...
        if self.book_advance_payments_in_separate_party_account:
            ...
            if entry:
                references = [x for x in self.get(
                    "references") if x.name == entry.name]
    ...
    def add_advance_gl_for_reference(self, gl_entries, invoice):
        ...
        if invoice.reconcile_effect_on:
            ...
            posting_date = get_reconciliation_effect_date(
                invoice, self.company, self.posting_date)
            frappe.db.set_value("Payment Entry Reference",
                                invoice.name, "reconcile_effect_on", posting_date)

        dr_or_cr, account = self.get_dr_and_account_for_advances(invoice)
        base_allocated_amount = self.calculate_base_allocated_amount_for_reference(
            invoice)
    ...
    def add_tax_gl_entries(self, gl_entries):
        for d in self.get("taxes"):
            ...
            if account_currency != self.company_currency:
                frappe.throw(_("Currency for {0} must be {1}").format(
                    d.account_head, self.company_currency))
            ...
            if not d.included_in_paid_amount:
                if get_account_currency(payment_account) != self.company_currency:
                    if self.payment_type == "Receive":
                        ...
                        exchange_rate = self.source_exchange_rate
                    base_tax_amount = flt(
                        (tax_amount / exchange_rate), self.precision("paid_amount"))
    ...
    def add_deductions_gl_entries(self, gl_entries):
        for d in self.get("deductions"):
            ...
            if account_currency != self.company_currency:
                frappe.throw(_("Currency for {0} must be {1}").format(
                    d.account, self.company_currency))
    ...
    def initialize_taxes(self):
        for tax in self.get("taxes"):
            ...
            tax_fields = ["total", "tax_fraction_for_current_item",
    ...
    def calculate_taxes(self):
        ...
        for i, tax in enumerate(self.get("taxes")):
            ...
            if i == 0:
                tax.total = flt(self.paid_amount_after_tax +
                                current_tax_amount, self.precision("total", tax))
            else:
                tax.total = flt(
                    self.get("taxes")[i - 1].total +
                    current_tax_amount, self.precision("total", tax)
                )
            ...
            if self.payment_type == "Pay":
                if tax.currency != self.paid_to_account_currency:
                    self.total_taxes_and_charges += flt(
                        current_tax_amount / self.target_exchange_rate)
                else:
                    self.total_taxes_and_charges += current_tax_amount
            elif self.payment_type == "Receive":
                if tax.currency != self.paid_from_account_currency:
                    self.total_taxes_and_charges += flt(
                        current_tax_amount / self.source_exchange_rate)
                else:
                    self.total_taxes_and_charges += current_tax_amount
    ...
    def get_current_tax_amount(self, tax):
        ...
        if tax.charge_type == "Actual":
            current_tax_amount = flt(
                tax.tax_amount, self.precision("tax_amount", tax))
        elif tax.charge_type == "On Paid Amount":
            current_tax_amount = (tax_rate / 100.0) * \
                self.paid_amount_after_tax
    ...
    def allocate_amount_to_references(self, paid_amount, paid_amount_change, allocate_payment_amount):
        ...
        if (self.payment_type == "Receive" and self.party_type == "Customer") or (
                self.payment_type == "Pay" and self.party_type in (
                    "Supplier", "Employee")
        ):
            ...
            if paid_amount > total_negative_outstanding:
                ...
                allocated_positive_outstanding = flt(
                    total_negative_outstanding - paid_amount, precision)
        ...
        def _allocation_to_unset_pr_row(
            ...
            if outstanding_amount > 0 and allocated_positive_outstanding >= 0:
                ...
                row.allocated_amount = min(
                    allocated_negative_outstanding, abs(outstanding_amount)) * -1
                allocated_negative_outstanding = flt(
                    allocated_negative_outstanding -
                    abs(row.allocated_amount), precision
                )
        ...
        if not paid_amount_change:
            ...
            allocate_open_payment_requests_to_references(
                self.references, self.precision("paid_amount"))
            ...
            references_outstanding_amounts = get_references_outstanding_amount(
                self.references) or {}
            ...
            for ref in self.references:
                ...
                if reference_outstanding_amount > 0 and allocated_positive_outstanding >= 0:
                    ...
                    remaining_references_allocated_amounts[key] = flt(
                        remaining_references_allocated_amounts[key] -
                        allocated_amount, precision
                    )
    ...
    def set_matched_payment_requests(self, matched_payment_requests):
        ...
        for ref in self.references:
            ...
            key = (ref.reference_doctype,
                   ref.reference_name, ref.allocated_amount)
...
def get_references_outstanding_amount(references=None):
    ...
    refs_with_payment_term = get_outstanding_of_references_with_payment_term(
        references) or {}
    refs_without_payment_term = get_outstanding_of_references_with_no_payment_term(
        references) or {}
...
def get_payment_request_outstanding_set_in_references(references=None):
    ...
    referenced_payment_requests = {
        row.payment_request for row in references if row.payment_request}
    ...
    if cint(getattr(tax, "included_in_paid_amount", None)):
        if tax.charge_type == "Actual":
            ...
            _on_previous_row_error(tax.row_id)
                ...
                [cint(t.included_in_paid_amount for t in doc.get(
                    "taxes")[: cint(tax.row_id) - 1])]
        ):
...
def get_outstanding_reference_documents(args, validate=False):
    ...
    if args.get("voucher_type") and args.get("voucher_no"):
        condition = " and voucher_type={} and voucher_no={}".format(
            frappe.db.escape(args["voucher_type"]), frappe.db.escape(
                args["voucher_no"])
        )
    ...
    if args.get("cost_center"):
        ...
        accounting_dimensions_filter.append(
            ple.cost_center == args.get("cost_center"))
    ...
    for dim in active_dimensions:
        if args.get(dim.fieldname):
            ...
            accounting_dimensions_filter.append(
                ple[dim.fieldname] == args.get(dim.fieldname))
    ...
    for fieldname, date_fields in date_fields_dict.items():
        if args.get(date_fields[0]) and args.get(date_fields[1]):
            ...
            posting_and_due_date.append(
                ple[fieldname].gte(args.get(date_fields[0])))
        elif args.get(date_fields[1]):
            # if only to date is supplied
            condition += f" and {fieldname} <= '{args.get(date_fields[1])}'"
            posting_and_due_date.append(
                ple[fieldname].lte(args.get(date_fields[1])))

    if args.get("company"):
        condition += " and company = {}".format(
        ...
        for d in outstanding_invoices:
            ...
            if d.voucher_type in ("Purchase Invoice"):
                d["bill_no"] = frappe.db.get_value(
                    d.voucher_type, d.voucher_no, "bill_no")
        ...
        if not validate:
            frappe.msgprint(
                _(
                    ...
                    _(ref_document_type), _(args.get("party_type")
                                            ).lower(), frappe.bold(args.get("party"))
                )
            )
        ...
        if entry.voucher_type in ["Sales Invoice", "Purchase Invoice"]:
            if payment_term_template := frappe.db.get_value(
                ...
                split_rows = get_split_invoice_rows(
                    entry, payment_term_template, exc_rates)
                if not split_rows:
                    continue
                ...
                if len(split_rows) > 1:
                    frappe.msgprint(
                        _("Splitting {0} {1} into {2} rows as per Payment Terms").format(
                            _(entry.voucher_type), frappe.bold(
                                entry.voucher_no), len(split_rows)
                        ),
                        alert=True,
                    )
...
def get_currency_data(outstanding_invoices: list, company: str | None = None) -> dict:
    ...
    company_currency = frappe.db.get_value(
        "Company", company, "default_currency") if company else None

    for doctype in ["Sales Invoice", "Purchase Invoice"]:
        invoices = [
            x.voucher_no for x in outstanding_invoices if x.voucher_type == doctype]
...
def get_split_invoice_rows(invoice: dict, payment_term_template: str, exc_rates: dict) -> list:
    ...
    for payment_term in payment_schedule:
        ...
        if not is_multi_currency_acc:
            payment_term_outstanding = doc_details.conversion_rate * \
                flt(payment_term.outstanding)
...
def get_party_details(company, party_type, party, date, cost_center=None):
    ...
    _party_name = "title" if party_type == "Shareholder" else party_type.lower() + \
        "_name"
    ...
    if party_type in ["Customer", "Supplier"]:
        ...
        bank_account = get_default_company_bank_account(
...
def get_reference_details(
    ...
    company_currency = ref_doc.get(
        "company_currency") or erpnext.get_company_currency(ref_doc.company)
    ...
    if reference_doctype == "Dunning":
        ...
        if not total_amount:
            if party_account_currency == company_currency:
                ...
                total_amount = ref_doc.get(
                    "rounded_total") or ref_doc.get("grand_total")
        ...
        if reference_doctype in ("Sales Invoice", "Purchase Invoice"):
            ...
            account = (
                ref_doc.get("debit_to") if reference_doctype == "Sales Invoice" else ref_doc.get(
                    "credit_to")
            )
        else:
            outstanding_amount = flt(total_amount) - \
                flt(ref_doc.get("advance_paid"))
...
def get_payment_entry(
    ...
    if dt in ("Sales Order", "Purchase Order") and flt(doc.per_billed, 2) >= (100.0 + over_billing_allowance):
        frappe.throw(
            _("Can only make payment against unbilled {0}").format(_(dt)))
    ...
    if party_type in ["Customer", "Supplier"] and not bank:
        party_bank_account = get_party_bank_account(
            party_type, doc.get(scrub(party_type)))
        if party_bank_account:
            account = frappe.db.get_value(
                "Bank Account", party_bank_account, "account")
    ...
    pe.paid_from_account_type = frappe.db.get_value(
        "Account", pe.paid_from, "account_type")
    pe.paid_to_account_type = frappe.db.get_value(
        "Account", pe.paid_to, "account_type")
    pe.paid_amount = paid_amount
    pe.received_amount = received_amount
    pe.letter_head = doc.get("letter_head")
    pe.bank_account = frappe.db.get_value(
        "Bank Account", {"is_company_account": 1,
                         "is_default": 1, "company": doc.company}, "name"
    )

    if dt in ["Purchase Order", "Sales Order", "Sales Invoice", "Purchase Invoice"]:
        pe.project = doc.get("project") or reduce(
            lambda prev, cur: prev or cur, [
                x.get("project") for x in doc.get("items")], None
    ...
    if doc.doctype == "Purchase Invoice" and doc.invoice_is_blocked():
        frappe.msgprint(_("{0} is on hold till {1}").format(
            doc.name, doc.release_date))
    else:
        if doc.doctype in (
                "Sales Invoice",
                "Purchase Invoice",
    ...
    if not created_from_payment_request:
        allocate_open_payment_requests_to_references(
            pe.references, pe.precision("paid_amount"))
...
def get_open_payment_requests_for_references(references=None):
    ...
    response = (
        ...
        .where(PR.status != "Paid")
        .where(PR.docstatus == 1)
        # to avoid old PRs with 0 outstanding amount
    ...
    for row in response:
        ...
        if key not in reference_payment_requests:
            reference_payment_requests[key] = {
                row.name: row.outstanding_amount}
        else:
...
def allocate_open_payment_requests_to_references(references=None, precision=None):
    ...
    references_open_payment_requests = get_open_payment_requests_for_references(
        references)
    ...
    while row_number <= len(references):
        ...
        reference_payment_requests = references_open_payment_requests.get(
            reference_key)
        ...
        payment_request, pr_outstanding_amount = next(
            iter(reference_payment_requests.items()))
        ...
        if pr_outstanding_amount == allocated_amount:
            ...
            allocated_amount = flt(
                allocated_amount - pr_outstanding_amount, precision)
...
def set_party_account(dt, dn, doc, party_type):
    if dt == "Sales Invoice":
        ...
        party_account = get_party_account(
            party_type, doc.get(party_type.lower()), doc.company)
...
def set_grand_total_and_outstanding_amount(party_amount, dt, party_account_currency, doc):
    ...
    if party_amount:
        ...
        outstanding_amount = doc.get("outstanding_amount") or (
            grand_total - flt(doc.advance_paid))
    ...
    if party_account_currency == bank.account_currency:
        ...
        if bank_amount:
            ...
            if bank and company_currency != bank.account_currency:
                ...
                posting_date = doc.get(
                    "posting_date") or doc.get("transaction_date")
...
def apply_early_payment_discount(paid_amount, received_amount, doc, party_account_currency, reference_date):
    ...
    has_payment_schedule = hasattr(
        doc, "payment_schedule") and doc.payment_schedule
    ...
    if doc.doctype in eligible_for_payments and has_payment_schedule:
        for term in doc.payment_schedule:
            if not term.discounted_amount and term.discount and reference_date <= term.discount_date:
                if term.discount_type == "Percentage":
                    grand_total = doc.get("grand_total") if is_multi_currency else doc.get(
                        "base_grand_total")
                ...
                conversion_rate = doc.get(
                    "conversion_rate", 1) if is_multi_currency else 1
                ...
                valid_discounts.append(
                    {"type": term.discount_type, "discount": term.discount})
                total_discount += discount_amount

        if total_discount:
            currency = doc.get(
                "currency") if is_multi_currency else doc.company_currency
            money = frappe.utils.fmt_money(total_discount, currency=currency)
            frappe.msgprint(
                _("Discount of {} applied as per Payment Term").format(money), alert=1)
...
def set_pending_discount_loss(pe, doc, discount_amount, base_total_discount_loss, party_account_currency):
    ...
    discount_amount = flt(
        discount_amount - base_total_discount_loss, doc.precision("grand_total"))
    ...
    if discount_amount > 0.0:
        ...
        book_tax_loss = frappe.db.get_single_value(
...
def split_early_payment_discount_loss(pe, doc, valid_discounts) -> float:
    ...
    base_loss_on_income = add_income_discount_loss(
        pe, doc, total_discount_percent)
...
def get_total_discount_percent(doc, valid_discounts) -> float:
    ...
    total_discount_percent = (
        sum(discount.get("discount")
            for discount in valid_discounts if discount.get("type") == "Percentage")
        or 0.0
    )
    ...
    total_discount_amount = (
        sum(discount.get("discount")
            for discount in valid_discounts if discount.get("type") == "Amount")
        or 0.0
    )

    if total_discount_amount:
        discount_percentage = (total_discount_amount /
                               doc.get("grand_total")) * 100
...
def add_income_discount_loss(pe, doc, total_discount_percent) -> float:
    ...
    base_loss_on_income = doc.get(
        "base_total") * (total_discount_percent / 100)
    positive_negative = -1 if pe.payment_type == "Pay" else 1
...
def add_tax_discount_loss(pe, doc, total_discount_percentage) -> float:
    ...
    for tax in doc.get("taxes", []):
        base_tax_loss = tax.get(
            "base_tax_amount_after_discount_amount") * (total_discount_percentage / 100)
...
def get_reference_as_per_payment_terms(
    ...
    for payment_term in payment_schedule:
        payment_term_outstanding = flt(
            payment_term.payment_amount -
            payment_term.paid_amount, payment_term.precision("payment_amount")
        )
        if not is_multi_currency_acc:
