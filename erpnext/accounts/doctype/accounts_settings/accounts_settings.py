...
class AccountsSettings(Document):
    ...
    if TYPE_CHECKING:
        ...
        determine_address_tax_category_from: DF.Literal["Billing Address",
                                                        "Shipping Address"]
        enable_common_party_accounting: DF.Check
        enable_fuzzy_matching: DF.Check
        enable_immutable_ledger: DF.Check
        enable_party_matching: DF.Check
        exchange_gain_loss_posting_date: DF.Literal["Invoice",
        ...
        receivable_payable_fetch_method: DF.Literal["Buffered Cursor",
                                                    "UnBuffered Cursor", "Raw SQL"]
        ...
        if self.has_value_changed("auto_reconciliation_job_trigger"):
            if (
                ...
                sync_auto_reconcile_config(
                    self.auto_reconciliation_job_trigger)
            else:
        ...
        if self.add_taxes_from_item_tax_template and self.add_taxes_from_taxes_and_charges_template:
            frappe.throw(
                _("You cannot enable both the settings '{0}' and '{1}'.").format(
                    frappe.bold(_(self.meta.get_label(
    ...
    def drop_ar_sql_procedures(self):
        ...
        frappe.db.sql(
            f"drop function if exists {InitSQLProceduresForAR.genkey_function_name}")
        frappe.db.sql(
            f"drop procedure if exists {InitSQLProceduresForAR.init_procedure_name}")
        frappe.db.sql(
            f"drop procedure if exists {InitSQLProceduresForAR.allocate_procedure_name}")
