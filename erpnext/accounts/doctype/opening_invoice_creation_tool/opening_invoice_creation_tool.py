...
class OpeningInvoiceCreationTool(Document):
    ...
    if TYPE_CHECKING:
        from erpnext.accounts.doctype.opening_invoice_creation_tool_item.opening_invoice_creation_tool_item import OpeningInvoiceCreationToolItem
    ...
    def onload(self):
        ...
        self.set_onload("temporary_opening_account",
                        get_temporary_opening_account(self.company))
    ...
    def get_opening_invoice_summary(self):
        ...
        company_wise_currency = {
            row.company: row.currency for row in companies}
            ...
            if self.create_missing_party:
                ...
                frappe.throw(
                    _("Row #{}: {} {} does not exist.").format(
                        row.idx, frappe.bold(
                            row.party_type), frappe.bold(row.party)
                    )
                )
    ...
    def add_party(self, party_type, party):
        ...
        if party_type == "Customer":
            ...
            if not supplier_group:
                frappe.throw(
                    _("Please Set Supplier Group in Buying Settings."))
    ...
    def get_invoice_dict(self, row=None):
        ...
        for dimension in accounting_dimension:
            invoice.update({dimension: self.get(
                dimension) or item.get(dimension)})
    ...
    def make_invoices(self):
        ...
        if len(invoices) < 50:
            ...
            if is_scheduler_inactive() and not frappe.flags.in_test:
                frappe.throw(_("Scheduler is inactive. Cannot import data."), title=_(
                    "Scheduler Inactive"))
...
def get_invoice_number():
    ...
    return f"{base_series}/{fiscal_year}"
...
def publish(index, total, doctype):
    frappe.publish_realtime(
        ...
        dict(
            ...
            message=_("Creating {} out of {} {}").format(
                index + 1, total, doctype),
            count=index + 1,
            total=total,
        ),
