...
class PurchaseInvoice(BuyingController):
    ...
    def validate(self):
        ...
        self.validate_multiple_billing(
        ...
        self.reset_default_field_value(
            "set_from_warehouse", "items", "from_warehouse")
    ...
    def validate_cash(self):
        if not self.cash_bank_account and flt(self.paid_amount):
            frappe.throw(
                _("Cash or Bank Account is mandatory for making payment entry"))
    ...
    def create_remarks(self):
        if not self.remarks:
            if self.bill_no:
                self.remarks = _(
                    "Against Supplier Invoice {0}").format(self.bill_no)
                if self.bill_date:
                    self.remarks += " " + \
                        _("dated {0}").format(formatdate(self.bill_date))
    ...
    def set_missing_values(self, for_validate=False):
        ...
        tds_category = frappe.db.get_value(
            "Supplier", self.supplier, "tax_withholding_category")
    ...
    def validate_with_previous_doc(self):
        ...
        if (
                cint(frappe.db.get_single_value(
                    "Buying Settings", "maintain_same_rate"))
    ...
    def validate_item_code(self):
        for d in self.get("items"):
            if not d.item_code:
                frappe.msgprint(_("Item Code required at Row No {0}").format(
                    d.idx), raise_exception=True)
    ...
    def set_expense_account(self, for_validate=False):
        ...
        for item in self.get("items"):
            ...
            if (
                        ...
                        if (
                            ...
                            msg = _(
                                ...
                                item.idx, frappe.bold(
                                    stock_not_billed_account), frappe.bold(item.item_code)
                            )
                            msg += "<br>"
                            msg += _(
                ...
                if not account:
                    ...
                    if not account:
                        form_link = get_link_to_form(
                            "Asset Category", item.asset_category)
                        throw(
                ...
                throw(_("Expense account is mandatory for item {0}").format(
                    item.item_code or item.item_name))

    def validate_expense_account(self):
        for item in self.get("items"):
            validate_account_head(
                item.idx, item.expense_account, self.company, "Expense")
    ...
    def force_set_against_expense_account(self):
        ...
        frappe.db.set_value(self.doctype, self.name,
                            "against_expense_account", self.against_expense_account)
        ...
        if frappe.db.get_value("Buying Settings", None, "po_required") == "Yes":
            ...
            for d in self.get("items"):
                if not d.purchase_order:
                    msg = _("Purchase Order Required for item {}").format(
                    ...
                    msg += _(
                        ...
                        frappe.bold(_("No")),
                        get_link_to_form("Buying Settings",
        ...
        if frappe.db.get_value("Buying Settings", None, "pr_required") == "Yes":
            ...
            for d in self.get("items"):
                if not d.purchase_receipt and d.item_code in stock_items:
                    msg = _("Purchase Receipt Required for item {}").format(
                        frappe.bold(d.item_code))
                    msg += "<br><br>"
                    msg += _(
    ...
    def check_prev_docstatus(self):
        for d in self.get("items"):
            if d.purchase_order:
                ...
                if not submitted:
                    frappe.throw(
                        _("Purchase Order {0} is not submitted").format(d.purchase_order))
            if d.purchase_receipt:
                ...
                if not submitted:
                    frappe.throw(_("Purchase Receipt {0} is not submitted").format(
                        d.purchase_receipt))
    ...
    def on_update_after_submit(self):
        ...
        child_tables = {"items": ("expense_account",),
                        "taxes": ("account_head",)}
    ...
    def make_gl_entries(self, gl_entries=None, from_repost=False):
        update_outstanding = "No" if (
            cint(self.is_paid) or self.write_off_account) else "Yes"
        if self.docstatus == 1:
            if not gl_entries:
    ...
    def update_supplier_outstanding(self, update_outstanding):
        if update_outstanding == "No":
            update_voucher_outstanding(
                ...
                voucher_no=self.return_against if cint(
                    self.is_return) and self.return_against else self.name,
    ...
    def get_gl_entries(self, warehouse_account=None):
        self.auto_accounting_for_stock = erpnext.is_perpetual_inventory_enabled(
            self.company)
    ...
    def check_asset_cwip_enabled(self):
        ...
        for item in self.get("items"):
            if item.item_code and item.is_fixed_asset:
                asset_category = frappe.get_cached_value(
                    "Item", item.item_code, "asset_category")
    ...
    def add_supplier_gl_entry(
        ...
        gl_entries.append(self.get_gl_dict(
            gl, self.party_account_currency, item=self))
        ...
        if self.update_stock:
            stock_ledger_entries = frappe.get_all(
                ...
                filters={"voucher_no": self.name,
                         "voucher_type": self.doctype, "is_cancelled": 0},
            )
                ...
                if (
                    ...
                    if item.from_warehouse:
                        ...
                        if self.is_internal_supplier and item.valuation_rate:
                            credit_amount = flt(
                                item.valuation_rate * item.stock_qty)
                    ...
                    if flt(item.rm_supp_cost):
                        ...
                        if not supplier_warehouse_account:
                            frappe.throw(
                                _("Please set account in Warehouse {0}").format(
                ...
                if item.purchase_receipt and valuation_tax_accounts:
                    ...
                    if not negative_expense_booked_in_pr:
                        ...
                        self.negative_expense_to_be_booked += flt(
                            item.item_tax_amount, item.precision(
                                "item_tax_amount")
                        )
    ...
    def get_provisional_accounts(self):
        ...
        linked_purchase_receipts = set(
            [d.purchase_receipt for d in self.items if d.purchase_receipt])
        ...
        pr_items = frappe.get_all(
            ...
            fields=["name", "provisional_expense_account",
                    "qty", "base_rate", "rate"],
        )
        default_provisional_account = self.get_company_default(
        ...
        rows_with_provisional_entries = [
            d.voucher_detail_no for d in provisional_gl_entries]
    ...
    def make_provisional_gl_entry(self, gl_entries, item):
        if item.purchase_receipt:
            ...
            if pr_item.get("has_provisional_entry"):
                purchase_receipt_doc = frappe.get_cached_doc(
    ...
    def make_stock_adjustment_entry(self, gl_entries, item, voucher_wise_stock_value, account_currency):
        ...
        if self.is_return and self.update_stock and (self.is_internal_supplier or not self.return_against):
            ...
            stock_amount = net_rate + item.item_tax_amount + \
                flt(item.landed_cost_voucher_amount)
            warehouse_debit_amount = flt(
                voucher_wise_stock_value.get(
                    (item.name, item.warehouse)), net_amt_precision
            )

            if flt(stock_amount, net_amt_precision) != flt(warehouse_debit_amount, net_amt_precision):
                cost_of_goods_sold_account = self.get_company_default(
            ...
            cost_of_goods_sold_account = self.get_company_default(
                "default_expense_account")
            stock_amount = flt(voucher_wise_stock_value.get(
                (item.name, item.warehouse)), net_amt_precision)
    ...
    def make_tax_gl_entries(self, gl_entries):
        ...
        for tax in self.get("taxes"):
            ...
            if (
                ...
                valuation_tax[tax.name] += (tax.add_deduct_tax ==
                                            "Add" and 1 or -1) * flt(base_amount)
                        ...
                        self.get_gl_dict(
                            {
                                ...
                                "credit_in_transaction_currency": flt(
                                    ...
                                    frappe.get_precision(
                                        "Purchase Invoice Item", "item_tax_amount"),
                                ),
                                "remarks": self.remarks or _("Accounting Entry for Stock"),
                            },
                            item=tax,
    ...
    def make_payment_gl_entries(self, gl_entries):
        ...
        if cint(self.is_paid) and self.cash_bank_account and self.paid_amount:
            bank_account_currency = get_account_currency(
    ...
    def update_project(self):
        ...
        for d in self.items:
            if d.project:
                if self.docstatus == 1:
                    projects[d.project] = projects.get(
                        d.project, 0) + d.base_net_amount
                elif self.docstatus == 2:
                    projects[d.project] = projects.get(
                        d.project, 0) - d.base_net_amount
        ...
        for proj, value in projects.items():
            res = frappe.qb.from_(pj).select(pj.total_purchase_cost).where(
                pj.name == proj).for_update().run()
    ...
    def validate_supplier_invoice(self):
        if self.bill_date:
            if getdate(self.bill_date) > getdate(self.posting_date):
                frappe.throw(
                    _("Supplier Invoice Date cannot be greater than Posting Date"))

        if self.bill_no:
            if cint(frappe.db.get_single_value("Accounts Settings", "check_supplier_invoice_uniqueness")):
                fiscal_year = get_fiscal_year(
                    self.posting_date, company=self.company, as_dict=True)
    ...
    def get_pr_details_billed_amt(self):
        ...
        pr_details = [d.get("pr_detail")
                      for d in self.get("items") if d.get("pr_detail")]
        if pr_details:
...
def get_purchase_document_details(doc):
    ...
    net_rate_map = frappe._dict(
        frappe.get_all(child_doctype, filters={"name": ("in", items)}, fields=[
                       "name", "net_rate"], as_list=1)
    )
...
# ============================================================
# Place at: apps/gajanand/gajanand/api/quick_debit_note.py
# ============================================================
#
# Creates and submits a Purchase Invoice return (Debit Note)
# for rate difference adjustment using DR ADJUST item.
#
# Mirror of quick_credit_note.py but for Purchase Invoice:
#   - is_return = 1
#   - return_against = original Purchase Invoice
#   - Copies taxes (CGST/SGST/IGST) from original PI
#   - Uses DR ADJUST item (create this Item if not exists)
#   - set_posting_time = 1 to preserve user-selected date
#   - Flags ignore_validate_return_items to allow DR ADJUST
#
# Prerequisites:
#   Create Item: DR ADJUST
#     - Item Name: RATE DIFFERENCE (PURCHASE)
...
@frappe.whitelist()
def create_quick_debit_note(return_against, posting_date, debit_amount, reason):
    """
    Create and submit a Quick Debit Note (Purchase Invoice return)
    for rate difference adjustment.

    - Copies all taxes (CGST/SGST/IGST) from original PI
    - GST entries will be created correctly on submit
    - Bypasses return item validation (DR ADJUST not in original PI)
    - Preserves posting_date via set_posting_time=1
    """

    # ── Fetch original Purchase Invoice ───────────────────────
    orig = frappe.get_doc("Purchase Invoice", return_against)

    if orig.docstatus != 1:
        frappe.throw("Original Purchase Invoice must be submitted.")

    if orig.is_return:
        frappe.throw(
            "Cannot create a Debit Note against another return document.")

    debit_amount = flt(debit_amount)
    if debit_amount <= 0:
        frappe.throw("Debit amount must be greater than zero.")

    # ── Ensure DR ADJUST item exists ──────────────────────────
    if not frappe.db.exists("Item", "DR ADJUST"):
        _create_dr_adjust_item()

    # ── Copy taxes from original PI ───────────────────────────
    # charge_type, account_head, rate, description all come intact
    taxes = []
    for t in orig.taxes:
        taxes.append({
            ...
            "add_deduct_tax":         t.add_deduct_tax or "Add",
        })
    ...
    items = [{
        "item_code":       "DR ADJUST",
        "item_name":       "RATE DIFFERENCE (PURCHASE)",
        "description":     reason,
        "qty": -1,          # positive qty, is_return makes it negative
        "rate":            debit_amount,
        "uom":             "Nos",
        "expense_account": first_item.get("expense_account", ""),
        "cost_center":     first_item.get("cost_center", orig.cost_center or ""),
        "warehouse":       first_item.get("warehouse", ""),
    }]

    # ── Create Purchase Invoice (is_return=1) ──────────────────
    ...
    dn.update({
        ...
        "set_posting_time":    1,          # preserve user-selected date
        ...
        "remarks":             reason,
        "credit_to":           orig.credit_to,
        "buying_price_list":   orig.buying_price_list,
        ...
        "update_stock":        0,          # debit note is accounting only

        # India Compliance: bill_no mandatory — copy from original or use prefixed ref
        "bill_no":             orig.bill_no or "DN-{}".format(return_against),
    ...
    # Skip return item validation — DR ADJUST not in original PI
    dn.flags.ignore_validate_return_items = True

    dn.insert(ignore_permissions=True)

    # Belt-and-suspenders: re-affirm posting_date after insert
    frappe.db.set_value("Purchase Invoice", dn.name, {
        "posting_date":     posting_date,
        "set_posting_time": 1,
    })
    frappe.db.commit()
...
def _create_dr_adjust_item():
    """Auto-create DR ADJUST item if it doesn't exist."""
    ...
    item.update({
        ...
        "is_purchase_item":   1,
        "is_sales_item":      0,
        "description":        "Used for purchase rate difference debit note adjustments",
    })
