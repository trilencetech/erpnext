...
def set_address_details(
    ...
    # party_billing
    party_billing_field = (
        "customer_address" if party_type in [
            "Lead", "Prospect"] else party_type.lower() + "_address"
    )

    party_details[party_billing_field] = party_address or get_default_address(
        party_type, party.name)
    if doctype:
        party_details.update(
            get_fetch_values(doctype, party_billing_field,
    ...
    if doctype:
        party_details.update(
            get_fetch_values(doctype, party_shipping_field,
    ...
    if doctype in SALES_TRANSACTION_TYPES and party_details.company_address:
        party_details.update(get_fetch_values(
            doctype, "company_address", party_details.company_address))
...
def get_party_account(party_type, party=None, company=None, include_advance=False):
    ...
    account = frappe.db.get_value(
        "Party Account", {"parenttype": party_type,
                          "parent": party, "company": company}, "account"
    )
    ...
    if not account and party_type in ["Customer", "Supplier"]:
        ...
        account = frappe.db.get_value(
            "Party Account",
            {"parenttype": party_group_doctype,
             "parent": group, "company": company},
            "account",
        )
...
def validate_party_accounts(doc):
    ...
    for account in doc.get("accounts"):
        ...
        validate_party_gle_currency(
            doc.doctype, doc.name, account.company, party_account_currency)
        ...
        if account.account:
            validate_account_head(
                account.idx, account.account, account.company)
        if account.advance_account:
            validate_account_head(
                account.idx, account.advance_account, account.company)
...
def get_due_date_from_template(template_name, posting_date, bill_date):
    ...
    for term in template.terms:
        if term.due_date_based_on == "Day(s) after invoice date":
            ...
            due_date = max(due_date, add_days(
                get_last_day(due_date), term.credit_days))
        else:
            due_date = max(due_date, get_last_day(
                add_months(due_date, term.credit_months)))
    return due_date
...
def validate_due_date(posting_date, due_date, bill_date=None, template_name=None, doctype=None):
    if getdate(due_date) < getdate(posting_date):
        ...
        validate_due_date_with_template(
            posting_date, due_date, bill_date, template_name, doctype)
...
def validate_due_date_with_template(posting_date, due_date, bill_date, template_name, doctype=None):
    ...
    default_due_date = format(get_due_date_from_template(
...
def validate_party_frozen_disabled(party_type, party_name):
    ...
    if party_type and party_name:
        if party_type in ("Customer", "Supplier"):
            party = frappe.get_cached_value(party_type, party_name, [
                                            "is_frozen", "disabled"], as_dict=True)
            if party.disabled:
                frappe.throw(_("{0} {1} is disabled").format(
...
def get_dashboard_info(party_type, party, loyalty_program=None):
    ...
    for d in company_wise_grand_total:
        company_wise_billing_this_year.setdefault(
            d.company, {"grand_total": d.grand_total,
    ...
    for d in companies:
        ...
        if party_account_currency == company_default_currency:
            billing_this_year = flt(company_wise_billing_this_year.get(
...
def get_partywise_advanced_payment_amount(
    ...
    party_ledger = party_ledger.where(
        acc.root_type == ("Liability" if account_type ==
                          "Payable" else "Asset")
    )
    ...
    advance_ledger = advance_ledger.where(
        acc.root_type == ("Asset" if account_type ==
                          "Payable" else "Liability")
    )
