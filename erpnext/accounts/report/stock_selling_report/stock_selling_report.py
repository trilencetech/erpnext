...
def get_columns():
    return [
        ...
        {
            "label": _("No. of Invoices"),   "fieldname": "invoice_count",
...
def get_data(filters):
    ...
    rows = frappe.db.sql(
        ...
        {where}
        GROUP BY sii.item_code, item.item_group
        ORDER BY item.item_group, sii.item_code
        """,
        values,
        as_dict=True,
    )
...
@frappe.whitelist()
def get_item_sales(item_code, filters):
    if isinstance(filters, str):
        filters = json.loads(filters)

    conditions = ["si.docstatus = 1", "si.is_return = 0", "sii.item_code = %(item_code)s"]
...
<head>
<meta charset="utf-8">
<title>Stock Selling Report – {company}</title>
<style>
  *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .doc-header {{ text-align: center; padding-bottom: 6px; border-bottom: 2px solid #166534; margin-bottom: 8px; }}
  .company-name {{ font-size: 17pt; font-weight: bold; letter-spacing: 3px; text-transform: uppercase; color: #166534; }}
  .report-title {{ font-size: 11pt; margin-top: 2px; color: #444; }}

  table {{ width: 100%; border-collapse: collapse; table-layout: fixed; }}
  col.c-code {{ width: 22%; }} col.c-grp {{ width: 18%; }} col.c-cnt {{ width: 14%; }}
  col.c-qty  {{ width: 20%; }} col.c-amt {{ width: 26%; }}

  thead th {{
    background: #166534; color: #fff; border: 1px solid #15803d;
    padding: 5px 7px; font-size: 10pt; font-weight: bold; text-align: left;
    -webkit-print-color-adjust: exact; print-color-adjust: exact;
  }}
  thead th.r {{ text-align: right; }}

  tbody td {{ border: 1px solid #bbf7d0; padding: 3.5px 7px; font-size: 10pt; vertical-align: middle; }}
  tbody td.r {{ text-align: right; }}
  tr.data-row:nth-child(even) td {{
    background: #f0fdf4; -webkit-print-color-adjust: exact; print-color-adjust: exact;
  }}

  tr.grand-total td {{
    background: #166534; color: #fff; border: 2px solid #166534;
...
<div class="doc-header">
  ...
  <div class="report-title">Stock Selling Report &nbsp;—&nbsp; {date_range}</div>
</div>
<table>
  <colgroup>
  ...
  <thead>
    <tr>
      ...
      <th class="r">INVOICES</th><th class="r">TOTAL QTY (KGS)</th>
    ...
    for row in data:
        ...
        if rtype == "data":
            html += (
                ...
                f'<td class="r">{row.get("invoice_count","")}</td>'
            ...
            html += (
                ...
                f'<td></td>'
                f'<td class="r"><strong>{row.get("invoice_count","")}</strong></td>'
                f'<td class="r"><strong>{row.get("total_qty",0):.3f}</strong></td>'
