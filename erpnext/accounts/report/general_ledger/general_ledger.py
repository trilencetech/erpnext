...
def validate_filters(filters, account_details):
    ...
    if not filters.get("categorize_by") and filters.get("group_by"):
        ...
        filters["categorize_by"] = filters["categorize_by"].replace(
        ...
        if filters.get("account"):
            ...
            gle_currency = frappe.db.get_value(
                ...
                {"party_type": filters.party_type,
                 "party": filters.party[0], "company": filters.company},
                "account_currency",
            )
                ...
                if filters.get(dimension.fieldname):
                    if frappe.get_cached_value("DocType", dimension.document_type, "is_tree"):
                        filters[dimension.fieldname] = get_dimension_with_children(
                            dimension.document_type, filters.get(
                                dimension.fieldname)
                        )
                        conditions.append(
...
def get_accounts_with_children(accounts):
    ...
    for account in accounts_data:
        conditions.append((doctype.lft >= account.lft) &
                          (doctype.rgt <= account.rgt))
...
def initialize_gle_map(gl_entries, filters, totals_dict):
    ...
    for gle in gl_entries:
        gle_map.setdefault(gle.get(group_by), _dict(
            totals=copy.deepcopy(totals_dict), entries=[]))
    return gle_map
...
def get_accountwise_gle(filters, accounting_dimensions, gl_entries, gle_map, totals):
    ...
    group_by = group_by_field(filters.get("categorize_by"))
    group_by_voucher_consolidated = filters.get(
    ...
    def update_value_in_dict(data, key, gle):
        ...
        if filters.get("show_net_values_in_party_account") and account_type_map.get(data[key].account) in (
            ...
            data[key][dr_or_cr +
                      "_in_account_currency"] = abs(net_value_in_account_currency)
    ...
    for gle in gl_entries:
        ...
        if gle.posting_date < from_date or (cstr(gle.is_opening) == "Yes" and not show_opening_entries):
            if not group_by_voucher_consolidated:
                update_value_in_dict(
                    gle_map[group_by_value].totals, "opening", gle)
                update_value_in_dict(
                    gle_map[group_by_value].totals, "closing", gle)
            ...
            if not group_by_voucher_consolidated:
                update_value_in_dict(
                    gle_map[group_by_value].totals, "total", gle)
                update_value_in_dict(
                    gle_map[group_by_value].totals, "closing", gle)
...
def get_account_type_map(company):
    account_type_map = frappe._dict(
        frappe.get_all("Account", fields=["name", "account_type"], filters={
...
def get_columns(filters):
    if filters.get("presentation_currency"):
        ...
        filters["presentation_currency"] = currency = get_company_currency(
            company)
    ...
    columns += [
        {"label": _("Voucher Type"),
         "fieldname": "voucher_type", "width": 120},
        {
    ...
    if filters.get("include_dimensions"):
        columns.append({"label": _("Project"), "options": "Project",
                       "fieldname": "project", "width": 100})

        for dim in get_accounting_dimensions(as_list=False):
            columns.append(
                {"label": _(dim.label), "options": dim.label,
                 "fieldname": dim.fieldname, "width": 100}
            )
        columns.append(
            {"label": _("Cost Center"), "options": "Cost Center",
             "fieldname": "cost_center", "width": 100}
        )

    columns.extend(
        [
            {"label": _("Against Voucher Type"),
             "fieldname": "against_voucher_type", "width": 100},
            {
            ...
            {"label": _("Supplier Invoice No"), "fieldname": "bill_no",
