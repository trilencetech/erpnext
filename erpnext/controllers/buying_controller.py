...
class BuyingController(SubcontractingController):
    def __setup__(self):
        self.flags.ignore_permlevel_for_fields = [
            "buying_price_list", "price_list_currency"]
    ...
    def create_package_for_transfer(self) -> None:
        ...
        if self.is_internal_transfer() and (
                self.doctype == "Purchase Receipt" or (
            ...
            for item in self.get("items"):
                if item.get(field) and not item.serial_and_batch_bundle and bundle_ids.get(item.get(field)):
                    item.serial_and_batch_bundle = self.make_package_for_transfer(
                        ...
                        and len(
                            sabe := frappe.get_all(
                                ...
                                filters={"parent": item.serial_and_batch_bundle, "serial_no": [
                                    "is", "not set"]},
                                pluck="name",
                            )
                        )
                    ...
                    frappe.set_value("Serial and Batch Entry",
                                     sabe[0], "qty", item.qty)
        ...
        if getattr(self, "supplier", None):
            self.update_if_missing(
                get_party_details(
                    ...
                    fetch_payment_terms_template=not self.get(
        ...
        if self.meta.get_field("supplier") and not self.supplier:
            for d in self.get("items"):
                supplier = frappe.db.get_value(
                    "Item Default", {"parent": d.item_code,
                                     "company": self.company}, "default_supplier"
                )
                if supplier:
                    self.supplier = supplier
                else:
                    item_group = frappe.db.get_value(
                        "Item", d.item_code, "item_group")
                    supplier = frappe.db.get_value(
                        "Item Default", {
    ...
    def validate_from_warehouse(self):
        for item in self.get("items"):
            if item.get("from_warehouse") and (item.get("from_warehouse") == item.get("warehouse")):
                frappe.throw(
                    _("Row #{0}: Accepted Warehouse and Supplier Warehouse cannot be same").format(
                        item.idx)
                )
    ...
    def set_supplier_address(self):
        ...
        for address_field, address_display_field in address_dict.items():
            if self.get(address_field):
                self.set(
                    address_display_field, render_address(
                        self.get(address_field), check_permissions=False)
                )
            ...
            if item.item_code and item.qty and item.item_code in stock_and_asset_items:
                ...
                if i == (last_item_idx - 1):
                    item.item_tax_amount = flt(
                        valuation_amount_adjustment, self.precision(
                            "item_tax_amount", item)
                    )
                else:
                    item.item_tax_amount = flt(
                        item_proportion *
                        total_valuation_amount, self.precision(
                            "item_tax_amount", item)
                    )
                ...
                if flt(item.conversion_factor) == 0.0:
                    item.conversion_factor = (
                        get_conversion_factor(item.item_code, item.uom).get(
        ...
        for d in self.get("items"):
            if not d.get(frappe.scrub(ref_doctype)):
                ...
                d.sales_incoming_rate = flt(
                    outgoing_rate * (d.conversion_factor or 1))
            else:
                field = "incoming_rate" if self.get(
                    "is_internal_supplier") else "rate"
                d.sales_incoming_rate = flt(
                    frappe.db.get_value(ref_doctype, d.get(
                        frappe.scrub(ref_doctype)), field)
    ...
    def validate_for_subcontracting(self):
        if self.is_subcontracted and self.get("is_old_subcontracting_flow"):
            if self.doctype in ["Purchase Receipt", "Purchase Invoice"] and not self.supplier_warehouse:
                frappe.throw(
                    _("Supplier Warehouse mandatory for sub-contracted {0}").format(self.doctype))

            for item in self.get("items"):
                if item in self.sub_contracted_items and not item.bom:
                    frappe.throw(
                        _("Please select BOM in BOM field for Item {0}").format(item.item_code))
    ...
    def validate_purchase_return(self):
        for d in self.get("items"):
            if self.is_return and flt(d.rejected_qty) != 0:
                frappe.throw(
                    _("Row #{0}: Rejected Qty can not be entered in Purchase Return").format(d.idx))
        ...
        for d in self.get("items"):
            if d.get(ref_fieldname):
                status = frappe.db.get_value(
                    ref_doctype, d.get(ref_fieldname), "status")
                if status in ("Closed", "On Hold"):
                    frappe.throw(_("{0} {1} is {2}").format(
                        ref_doctype, d.get(ref_fieldname), status))
    ...
    def update_stock_ledger(self, allow_negative_stock=False, via_landed_cost_voucher=False):
        ...
        for d in self.get("items"):
            ...
            if d.warehouse:
                pr_qty = flt(flt(d.qty) * flt(d.conversion_factor),
                             d.precision("stock_qty"))
                    ...
                    if d.from_warehouse and (
                        ...
                        from_warehouse_sle = self.get_sl_entries(
                            d,
                            {
                                ...
                                "recalculate_rate": 1,
                                "serial_and_batch_bundle": (
                                    self.get_package_for_target_warehouse(
                                        d, d.from_warehouse, "Inward")
        ...
        for sle in sl_entries:
            child = frappe.get_value(
                "Purchase Receipt Item", sle.voucher_detail_no, "item_id")
            if child:
                sle.item_id = child
    ...
    def update_ordered_and_reserved_qty(self):
        ...
        for d in self.get("items"):
            if self.doctype == "Purchase Receipt" and d.purchase_order:
                po_map.setdefault(d.purchase_order, []).append(
                    ...
                    if item_data.get("asset_naming_series"):
                        ...
                        if item_data.get("is_grouped_asset"):
                            asset = self.make_asset(
                                d, accounting_dimensions, is_grouped_asset=True)
                            created_assets.append(asset)
                        else:
                            for _qty in range(cint(d.qty)):
                                asset = self.make_asset(
                                    d, accounting_dimensions)
                                created_assets.append(asset)
                        ...
                        if len(created_assets) > 5:
                            ...
                            messages.append(
                                _("{} Assets created for {}").format(
                                    len(created_assets), frappe.bold(
                                        d.item_code)
                                )
                            )
                        else:
                            assets_link = list(
                                map(lambda d: frappe.utils.get_link_to_form(
                                    "Asset", d), created_assets)
                            )
    ...
    def make_asset(self, row, accounting_dimensions, is_grouped_asset=False):
        if not row.asset_location:
            frappe.throw(_("Row {0}: Enter location for the asset item {1}").format(
        ...
        for d in self.get("items"):
            if d.is_fixed_asset:
                ...
                assets = frappe.db.get_all(
                    "Asset", filters={field: self.name, "item_code": d.item_code})
                ...
                for asset in assets:
                    ...
                    if delete_asset and is_auto_create_enabled:
                        ...
                        for movement in movements:
                            frappe.delete_doc(
                                "Asset Movement", movement.name, force=1)
    ...
    def delete_linked_asset(self):
        ...
        asset_movement = frappe.db.get_value(
            "Asset Movement", {"reference_name": self.name}, "name")
        ...
        if self.schedule_date:
            for d in self.get("items"):
                ...
                if (
                    ...
                    frappe.throw(
                        _("Row #{0}: Reqd by Date cannot be before Transaction Date").format(d.idx))
        else:
...
def get_asset_item_details(asset_items):
    ...
    for d in frappe.get_all(
            "Item",
            fields=["name", "auto_create_assets",
                    "asset_naming_series", "is_grouped_asset"],
