...
class SellingController(StockController):
    ...
    def validate_standalone_serial_nos_customer(self):
        ...
        if self.doctype in ["Sales Invoice", "Delivery Note"]:
            bundle_ids = [d.serial_and_batch_bundle for d in self.get(
                "items") if d.serial_and_batch_bundle]
            if not bundle_ids:
                return
    ...
    def set_missing_lead_customer_details(self, for_validate=False):
        ...
        if customer:
            ...
            self.update_if_missing(
                get_lead_details(
                    lead,
                    posting_date=self.get(
                        "transaction_date") or self.get("posting_date"),
                    company=self.company,
                )
            )
    ...
    def set_total_in_words(self):
        ...
        if self.meta.get_field("base_in_words"):
            ...
            self.base_in_words = money_in_words(
                base_amount, self.company_currency)

        if self.meta.get_field("in_words"):
            amount = abs(
                self.grand_total if self.is_rounded_total_disabled() else self.rounded_total)
        ...
        for sales_person in sales_team:
            ...
            if sales_person.commission_rate:
                sales_person.incentives = flt(
                    ...
                    flt(sales_person.commission_rate) / 100.0,
    ...
    def validate_sales_team(self, sales_team):
        ...
        for row in sales_person_status:
            if not row.enabled:
                frappe.throw(
                    _("Sales Person <b>{0}</b> is disabled.").format(row.name))

    def validate_max_discount(self):
        for d in self.get("items"):
            if d.item_code:
                discount = flt(frappe.get_cached_value(
                    "Item", d.item_code, "max_discount"))

                if discount and flt(d.discount_percentage) > discount:
                    frappe.throw(_("Maximum discount for Item {0} is {1}%").format(
    ...
    def validate_selling_price(self):
        ...
        for item in self.items:
            ...
            if flt(item.base_net_rate) < flt(last_purchase_rate_in_sales_uom):
                throw_message(item.idx, item.item_name,
                              last_purchase_rate_in_sales_uom, "last purchase rate")
        ...
        for rate in valuation_rates:
            valuation_rate_map[(rate.item_code, rate.warehouse)
                               ] = rate.valuation_rate
    ...
    def _fetch_product_bundle_items(self, item_code):
        ...
        items_to_fetch = {
            row.item_code for row in self.items if row.item_code not in product_bundle_items}
        ...
        items_with_product_bundle = {
            ...
            for row in frappe.get_all(
                "Product Bundle",
                filters={"new_item_code": (
                    "in", items_to_fetch), "disabled": 0},
        ...
        for d in self.get("items"):
            if d.so_detail:
                if self.doctype == "Delivery Note" and d.against_sales_order:
                    so_map.setdefault(d.against_sales_order,
                                      []).append(d.so_detail)
        ...
        for d in items:
            ...
            if not self.get("return_against") or (
                    get_valuation_method(
                        d.item_code) == "Moving Average" and self.get("is_return")
            ):
                # Get incoming rate based on original item cost based on valuation method
                qty = flt(d.get("stock_qty") or d.get(
                    "actual_qty") or d.get("qty"))
                ...
                if self.is_internal_transfer():
                    if self.doctype == "Delivery Note" or self.get("update_stock"):
                        if d.doctype == "Packed Item":
                            incoming_rate = flt(
                                flt(d.incoming_rate, d.precision(
                                    "incoming_rate")) * d.conversion_factor,
    ...
    def update_stock_ledger(self, allow_negative_stock=False):
        ...
        doc_name = self.doctype
        for sle in sl_entries:
            child = frappe.get_value(
                doc_name+" Item", sle.voucher_detail_no, "item_id")
            if child:
                sle.item_id = child
    ...
    def get_sle_for_target_warehouse(self, item_row):
        sle = self.get_sl_entries(
            item_row, {"actual_qty": flt(
                item_row.qty), "warehouse": item_row.target_warehouse}
        )
    ...
    def set_pos_for_sales_invoice(self):
        ...
        self.po_no = ", ".join(list(set(x.strip()
                               for x in ",".join(po_nos).split(","))))
    ...
    def get_po_nos(self, ref_doctype, ref_fieldname, po_nos):
        doc_list = list(set(d.get(ref_fieldname)
                        for d in self.items if d.get(ref_fieldname)))
        if doc_list:
            po_nos += [
                d.po_no
    ...
    def validate_target_warehouse(self):
        ...
        if not self.get("is_internal_customer") and any(d.get("target_warehouse") for d in items):
            ...
            msg += " " + \
                _("This {} will be treated as material transfer.").format(
                    _(self.doctype))
                ...
                for sre in sre_list:
                    ...
                    if sre_doc.reservation_based_on == "Serial and Batch" and item.serial_and_batch_bundle:
                        ...
                        if sre_doc.has_serial_no:
                            ...
                            for entry in sre_doc.sb_entries:
                                if entry.serial_no in delivered_serial_nos:
                                    # Qty will always be 0 or 1 for Serial No.
                                    entry.delivered_qty = 1
                                    entry.db_update()
                                    qty_can_be_deliver += 1
                                    delivered_serial_nos.remove(
                                        entry.serial_no)
                        else:
                            delivered_batch_qty = {
                            ...
                            for entry in sre_doc.sb_entries:
                                if entry.batch_no in delivered_batch_qty:
                                    delivered_qty = min(
                                        (entry.qty -
                                         entry.delivered_qty), delivered_batch_qty[entry.batch_no]
                                    )
                        ...
                        qty_can_be_deliver = min(
                            (sre_doc.reserved_qty -
                             sre_doc.delivered_qty), qty_to_deliver
                        )
                        ...
                        if sre_doc.has_serial_no:
                            ...
                            for entry in sre_doc.sb_entries:
                                if entry.serial_no in serial_nos_to_undelivered:
                                    ...
                                    serial_nos_to_undelivered.remove(
                                        entry.serial_no)
                        else:
    ...
    for d in obj.get("items"):
        if d.item_code:
            if getattr(d, "income_account", None):
                set_item_default(d.item_code, obj.company,
...
def get_serial_and_batch_bundle(child, parent, delivery_note_child=None):
    ...
    item_details = frappe.db.get_value("Item", child.item_code, [
