...
class Customer(TransactionBase):
    ...
    def load_dashboard_info(self):
        info = get_dashboard_info(
            self.doctype, self.name, self.loyalty_program)
    ...
    def validate(self):
        ...
        self.set_payment_terms_template()
        ...
        if self.sales_team:
            if sum(member.allocated_percentage or 0 for member in self.sales_team) != 100:
                frappe.throw(
                    _("Total contribution percentage should be equal to 100"))
    ...
    def validate_default_bank_account(self):
        if self.default_bank_account:
            ...
            if not is_company_account:
                frappe.throw(
                    _("{0} is not a company bank account").format(
                        frappe.bold(self.default_bank_account))
                )
    ...
    def set_payment_terms_template(self):
        if not self.credit_days:
            return

        # Check if template exists
        template_name = f"{self.credit_days} Days Credit"
        existing = frappe.db.exists("Payment Terms Template", template_name)
        ...
        # Link template to customer
        self.payment_terms = template_name
    ...
    def update_customer_groups(self):
        ignore_doctypes = ["Lead", "Opportunity",
                           "POS Profile", "Tax Rule", "Pricing Rule"]
        ...
        if self.lead_name:
            ...
            for row in linked_contacts_and_addresses:
                ...
                if not linked_doc.has_link("Customer", self.name):
                    linked_doc.append("links", dict(
                        link_doctype="Customer", link_name=self.name))
                    linked_doc.save(
    ...
    def validate_credit_limit_on_change(self):
        ...
        current_credit_limits = [d.credit_limit for d in sorted(
            self.credit_limits, key=lambda k: k.company)]
        ...
        for limit in self.credit_limits:
            if limit.company in company_record:
                frappe.throw(
                    _("Credit limit is already defined for the Company {0}").format(
                        limit.company, self.name)
                )
            else:
    ...
    def on_trash(self):
        ...
        if self.lead_name:
            frappe.db.sql(
                "update `tabLead` set status='Interested' where name=%s", self.lead_name)
...
def send_emails(args):
    ...
    subject = _("Credit limit reached for customer {0}").format(
        args.get("customer"))
    message = _("Credit limit has been crossed for customer {0} ({1}/{2})").format(
        args.get("customer"), args.get(
            "customer_outstanding"), args.get("credit_limit")
    )
    frappe.sendmail(recipients=args.get(
        "credit_controller_users_list"), subject=subject, message=message)
...
def get_customer_outstanding(customer, company, ignore_outstanding_sales_order=False, cost_center=None):
    ...
    if not ignore_outstanding_sales_order:
        ...
        outstanding_based_on_so = flt(
            outstanding_based_on_so[0][0]) if outstanding_based_on_so else 0
    ...
    si_amounts = frappe.db.sql(
		...
		GROUP BY dn_detail""".format(
            ", ".join(frappe.db.escape(dn_item.name)
        ...
        if not credit_limit:
            ...
            result = frappe.db.get_values(
                ...
                {"parent": customer_group,
                 "parenttype": "Customer Group", "company": company},
