...
class SalesOrder(SellingController):
    ...
    def validate_delivery_date(self):
        if self.order_type == "Sales" and not self.skip_delivery_note:
            ...
            max_delivery_date = max(
                delivery_date_list) if delivery_date_list else None
            if (max_delivery_date and not self.delivery_date) or (
                    max_delivery_date and getdate(
                        self.delivery_date) != getdate(max_delivery_date)
            ):
            ...
            if (
                    (
                        frappe.get_cached_value(
                            "Item", d.item_code, "is_stock_item") == 1
                        or (
    ...
    def update_enquiry_status(self, prevdoc, flag):
        ...
        if enq:
            frappe.db.sql(
                "update `tabOpportunity` set status = %s where name=%s", (flag, enq[0][0]))
        ...
        for quotation in set(d.prevdoc_docname for d in self.get("items")):
            if quotation:
                ...
                if doc.docstatus.is_cancelled():
                    frappe.throw(
                        _("Quotation {0} is cancelled").format(quotation))

                doc.set_status(update=True)
                doc.update_opportunity(
                    "Converted" if flag == "submit" else "Quotation")

    def validate_drop_ship(self):
        for d in self.get("items"):
            if d.delivered_by_supplier and not d.supplier:
                frappe.throw(_("Row #{0}: Set Supplier for item {1}").format(
                    d.idx, d.item_code))
    ...
    def check_credit_limit(self):
        ...
        if not cint(
                frappe.db.get_value(
                    ...
                    {"parent": self.customer, "parenttype": "Customer",
    ...
    def check_nextdoc_docstatus(self):
        ...
        if linked_invoices:
            linked_invoices = [get_link_to_form(
                "Sales Invoice", si) for si in linked_invoices]
            frappe.throw(
    ...
    def check_modified_date(self):
        ...
        date_diff = frappe.db.sql(
            f"select TIMEDIFF('{mod_db}', '{cstr(self.modified)}')")
        if date_diff and date_diff[0][0]:
            frappe.throw(_("{0} {1} has been modified. Please refresh.").format(
            ...
            if item.supplier:
                supplier = frappe.db.get_value(
                    "Sales Order Item", {"parent": self.name,
                                         "item_code": item.item_code}, "supplier"
                )
    ...
    def update_delivery_status(self):
        ...
        for item in self.items:
            if item.delivered_by_supplier:
                ...
                item.db_set("delivered_qty", flt(
                    item_delivered_qty), update_modified=False)

            delivered_qty += min(item.delivered_qty, item.qty)
            tot_qty += item.qty

        if tot_qty != 0:
            self.db_set("per_delivered", flt(delivered_qty /
                        tot_qty) * 100, update_modified=False)
    ...
    def update_picking_status(self):
        ...
        for so_item in self.items:
            if cint(
                    frappe.get_cached_value(
                        "Item", so_item.item_code, "is_stock_item")
        ...
        if total_picked_qty and total_qty:
            ...
            pick_percentage = frappe.db.get_single_value(
                "Stock Settings", "over_picking_allowance")
            if pick_percentage:
    ...
    def on_recurring(self, reference_doc, auto_repeat_doc):
        ...
        for d in self.get("items"):
            reference_delivery_date = frappe.db.get_value(
                "Sales Order Item",
                {"parent": reference_doc.name,
                 "item_code": d.item_code, "idx": d.idx},
                "delivery_date",
            )
...
def get_requested_item_qty(sales_order):
    ...
    for d in frappe.db.get_all(
            ...
            fields=["sales_order_item",
                    "sum(qty) as qty", "sum(received_qty) as received_qty"],
    ...
    def get_remaining_qty(so_item):
        return flt(
            ...
            - max(
                ...
                - flt(requested_item_qty.get(so_item.name,
                                             {}).get("received_qty")),
                0,
            )
        )
...
def make_delivery_note(source_name, target_doc=None, kwargs=None):
    ...
    def set_missing_values(source, target):
        ...
        if target.company_address:
            target.update(get_fetch_values("Delivery Note",
    ...
    def update_item(source, target, source_parent):
        target.base_amount = (
            flt(source.qty) - flt(source.delivered_qty)) * flt(source.base_rate)
        target.amount = (flt(source.qty) -
                         flt(source.delivered_qty)) * flt(source.rate)
        target.qty = (
            flt(source.qty) if is_unit_price_row(source) else flt(
                source.qty) - flt(source.delivered_qty)
        )
    ...
    if not kwargs.skip_item_mapping and kwargs.for_reserved_stock:
        ...
        if sre_list:
            ...
            for sre in sre_list:
                ...
                dn_item.qty = flt(sre.reserved_qty) / \
                    flt(dn_item.get("conversion_factor", 1))

                if sre.reservation_based_on == "Serial and Batch" and (sre.has_serial_no or sre.has_batch_no):
                    dn_item.serial_and_batch_bundle = get_ssb_bundle_for_voucher(
                        sre)
...
def make_sales_invoice(source_name, target_doc=None, ignore_permissions=False):
    ...
    def set_missing_values(source, target):
        ...
        if target.company_address:
            target.update(get_fetch_values("Sales Invoice",
                          "company_address", target.company_address))
        ...
        target.debit_to = get_party_account(
            "Customer", source.customer, source.company)
...
def make_purchase_order_for_default_supplier(source_name, selected_items=None, target_doc=None):
    ...
    def set_missing_values(source, target):
        ...
        default_payment_terms = frappe.get_value(
...
def make_work_orders(items, sales_order, company, project=None):
    ...
    for i in items:
        if not i.get("bom"):
            frappe.throw(_("Please select BOM against item {0}").format(
                i.get("item_code")))
        if not i.get("pending_qty"):
            frappe.throw(_("Please select Qty against item {0}").format(
                i.get("item_code")))
...
def create_pick_list(source_name, target_doc=None):
    ...
    def update_item_quantity(source, target, source_parent) -> None:
        picked_qty = flt(source.picked_qty) / \
            (flt(source.conversion_factor) or 1)
        qty_to_be_picked = flt(source.qty) - \
            max(picked_qty, flt(source.delivered_qty))
    ...
    def update_packed_item_qty(source, target, source_parent) -> None:
        ...
        for item in source_parent.items:
            if source.parent_detail_docname == item.name:
                picked_qty = flt(item.picked_qty) / \
                    (flt(item.conversion_factor) or 1)
                pending_percent = (
                    item.qty - max(picked_qty, item.delivered_qty)) / item.qty
        ...
        product_bundle_parents = [
            ...
            for pb in frappe.get_all(
                "Product Bundle", {"new_item_code": [
                    "in", item_codes], "disabled": 0}, ["new_item_code"]
            )
        ]
