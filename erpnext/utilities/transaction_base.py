...
class TransactionBase(StatusUpdater):
    ...
    def validate_with_previous_doc(self, ref):
        self.exclude_fields = ["conversion_factor",
                               "uom"] if self.get("is_return") else []
        ...
        for key, val in ref.items():
            ...
            for d in self.get_all_children(self.doctype + " Item"):
                ...
                if ref_dn:
                    if is_child:
                        self.compare_values(
                            {key: [ref_dn]}, val["compare_fields"], d)
                        if ref_dn not in item_ref_dn:
                            item_ref_dn.append(ref_dn)
                        elif not val.get("allow_duplicate_prev_row_id"):
                            frappe.throw(
                                _("Duplicate row {0} with same {1}").format(d.idx, key))
                    elif ref_dn:
        ...
        for reference_doctype, ref_dn_list in ref_doc.items():
            ...
            for reference_name in ref_dn_list:
                ...
                if not prevdoc_values:
                    frappe.throw(_("Invalid reference {0} {1}").format(
    ...
    def validate_rate_with_reference_doc(self, ref_details):
        ...
        buying_doctypes = ["Purchase Order",
        ...
        for ref_dt, ref_dn_field, ref_link_field in ref_details:
            reference_names = [d.get(ref_link_field) for d in self.get(
                "items") if d.get(ref_link_field)]
            reference_details = self.get_reference_details(
                reference_names, ref_dt + " Item")
    ...
    def get_link_filters(self, for_doctype):
        if hasattr(self, "prev_link_mapper") and self.prev_link_mapper.get(for_doctype):
            ...
            values = filter(None, tuple(
                item.as_dict()[fieldname] for item in self.items))

            if values:
                ret = {for_doctype: {"filters": [
                    [for_doctype, "name", "in", values]]}}
            else:
                ret = None
        else:
            ret = None
        ...
        if self.doctype in ["Customer", "Supplier"]:
            ...
            for x in self.accounts:
                company_default_currency = frappe.get_cached_value(
                    "Company", x.company, "default_currency")
...
def validate_uom_is_integer(doc, uom_field, qty_fields, child_dt=None):
    ...
    distinct_uoms = tuple(set(uom for uom in (d.get(uom_field)
                          for d in doc.get_all_children()) if uom))
    integer_uoms = set(
        d[0]
