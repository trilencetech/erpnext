...
def make_sl_entries(sl_entries, allow_negative_stock=False, via_landed_cost_voucher=False):
    ...
    if sl_entries:
        ...
        if cancel:
            ...
            set_as_cancel(sl_entries[0].get(
                "voucher_type"), sl_entries[0].get("voucher_no"))
        ...
        for sle in sl_entries:
            ...
            if sle.get("actual_qty") or sle.get("voucher_type") == "Stock Reconciliation":
                sle_doc = make_entry(
                    sle, allow_negative_stock, via_landed_cost_voucher)

            args = sle_doc.as_dict()
            args["posting_datetime"] = get_combine_datetime(
                args.posting_date, args.posting_time)

            if sle.get("voucher_type") == "Stock Reconciliation":
                # preserve previous_qty_after_transaction for qty reposting
                args.previous_qty_after_transaction = sle.get(
                    "previous_qty_after_transaction")

            is_stock_item = frappe.get_cached_value(
                "Item", args.get("item_code"), "is_stock_item")
            if is_stock_item:
                bin_name = get_or_make_bin(
                    args.get("item_code"), args.get("warehouse"))
                args.reserved_stock = flt(frappe.db.get_value(
                    "Bin", bin_name, "reserved_stock"))
                repost_current_voucher(
                    args, allow_negative_stock, via_landed_cost_voucher)
...
def validate_serial_no(sle):
    ...
    for sn in get_serial_nos(sle.serial_no):
        ...
        for row in get_stock_ledger_entries(args, ">"):
            ...
            voucher_no = frappe.bold(get_link_to_form(
        ...
        if vouchers:
            ...
            title = "Cannot Submit" if not sle.get(
                "is_cancelled") else "Cannot Cancel"
            frappe.throw(_(msg), title=_(title),
                         exc=SerialNoExistsInFutureTransaction)
    ...
    if kargs[0].get("is_cancelled"):
        ...
        if repost_entry:
            ...
            if repost_entry.status == "Queued":
                doc = frappe.get_doc(
                    "Repost Item Valuation", repost_entry.name)
                doc.status = "Skipped"
...
def repost_future_sle(
    ...
    distinct_item_warehouses = get_distinct_item_warehouse(
        args, doc, reposting_data=reposting_data)
    affected_transactions = get_affected_transactions(
        doc, reposting_data=reposting_data)
        ...
        for i, d in enumerate(args):
            distinct_item_warehouses.setdefault(
                (d.item_code, d.warehouse), frappe._dict(
                    {"reposting_status": False, "sle": d, "args_idx": i})
            )
...
class update_entries_after:
    ...
    def __init__(
        ...
        self.company = frappe.get_cached_value(
            "Warehouse", self.args.warehouse, "company")
        self.set_precision()
        self.valuation_method = get_valuation_method(self.item_code)

        self.new_items_found = False
        self.distinct_item_warehouses = args.get(
            "distinct_item_warehouses", frappe._dict())
    ...
    def set_precision(self):
        self.flt_precision = cint(
            frappe.db.get_default("float_precision")) or 2
        ...
        if self.args.get("sle_id"):
            ...
            while i < len(entries_to_fix):
                ...
                if sle.dependant_sle_voucher_detail_no:
                    entries_to_fix = self.get_dependent_entries_to_fix(
                        entries_to_fix, sle)
    ...
    def update_distinct_item_warehouses(self, dependant_sle):
        ...
        if key not in self.distinct_item_warehouses:
            ...
            existing_sle_posting_date = self.distinct_item_warehouses[key].get(
                "sle", {}).get("posting_date")

            dependent_voucher_detail_nos = self.get_dependent_voucher_detail_nos(
                key)
            if getdate(dependant_sle.posting_date) < getdate(existing_sle_posting_date):
                ...
                dependent_voucher_detail_nos.append(
    ...
    def is_dependent_voucher_reposted(self, dependant_sle) -> bool:
        ...
        if self.args.items_to_be_repost and self.args.current_index:
            ...
            while index < len(self.args.items_to_be_repost):
                if (
                        self.args.items_to_be_repost[index].get(
                            "item_code") == dependant_sle.item_code
    ...
    def process_sle(self, sle):
        ...
        if (
            ...
            sle.outgoing_rate = get_incoming_rate_for_inter_company_transfer(
                sle)
    ...
    def get_serialized_values(self, sle):
        ...
        if actual_qty > 0:
            ...
            if not sle.is_cancelled:
                ...
                new_sle.serial_nos = get_serial_nos_data(
                    new_sle.get("serial_no"))
    ...
    def reset_actual_qty_for_stock_reco(self, sle):
        ...
        doc.recalculate_current_qty(
            sle.voucher_detail_no, sle.creation, sle.actual_qty > 0)

        if sle.actual_qty < 0:
            sle.actual_qty = (
                flt(frappe.db.get_value("Stock Reconciliation Item",
                    sle.voucher_detail_no, "current_qty"))
                * -1
            )
    ...
    def calculate_valuation_for_serial_batch_bundle(self, sle):
        ...
        if self.args.get("sle_id") and sle.actual_qty < 0:
            ...
            doc = frappe.get_doc("Serial and Batch Bundle",
                                 sle.serial_and_batch_bundle)
            doc.set_incoming_rate(
                save=True, allow_negative_stock=self.allow_negative_stock)
            doc.calculate_qty_and_amount(save=True)

        self.wh_data.stock_value = round_off_if_near_zero(
            self.wh_data.stock_value + doc.total_amount)
        self.wh_data.qty_after_transaction += flt(
            doc.total_qty, self.flt_precision)
    ...
    def validate_negative_stock(self, sle):
        ...
        """
        diff = self.wh_data.qty_after_transaction + \
            flt(sle.actual_qty) - flt(self.reserved_stock)
    ...
    def get_incoming_outgoing_rate_from_transaction(self, sle):
        ...
        if sle.voucher_type == "Stock Entry":
            self.recalculate_amounts_in_stock_entry(
                sle.voucher_no, sle.voucher_detail_no)
            rate = frappe.db.get_value(
                "Stock Entry Detail", sle.voucher_detail_no, "valuation_rate")
        # Sales and Purchase Return
            ...
            if frappe.get_cached_value(sle.voucher_type, sle.voucher_no, "is_return"):
                ...
                item_code, incoming_rate = frappe.db.get_value(
                    sle.voucher_type +
                    " Item", sle.voucher_detail_no, ["item_code", rate_field]
                )
                ...
                if item_code == sle.item_code:
                    ...
                    rate = frappe.db.get_value(
                        ref_doctype,
                        {"parent_detail_docname": sle.voucher_detail_no,
                            "item_code": sle.item_code},
                        rate_field,
                    )
    ...
    def update_outgoing_rate_on_transaction(self, sle):
        ...
        if sle.actual_qty and sle.voucher_detail_no:
            outgoing_rate = abs(
                flt(sle.stock_value_difference)) / abs(sle.actual_qty)
            ...
            if flt(sle.actual_qty) < 0 and sle.voucher_type == "Stock Entry":
                ...
                self.update_rate_on_delivery_and_sales_return(
                    sle, outgoing_rate)
    ...
    def update_rate_on_stock_entry(self, sle, outgoing_rate):
        frappe.db.set_value("Stock Entry Detail",
                            sle.voucher_detail_no, "basic_rate", outgoing_rate)
    ...
    def is_manufacture_entry_with_sabb(self, sle):
        if (
            ...
            purpose = frappe.get_cached_value(
                "Stock Entry", sle.voucher_no, "purpose")
    ...
    def recalculate_amounts_in_stock_entry(self, voucher_no, voucher_detail_no):
        stock_entry = frappe.get_doc(
            "Stock Entry", voucher_no, for_update=True)
    ...
    def update_rate_on_delivery_and_sales_return(self, sle, outgoing_rate):
        ...
        item_code = frappe.db.get_value(
            sle.voucher_type + " Item", sle.voucher_detail_no, "item_code")
        if item_code == sle.item_code:
            frappe.db.set_value(
    ...
    def update_rate_on_subcontracting_receipt(self, sle, outgoing_rate):
        if frappe.db.exists("Subcontracting Receipt Item", sle.voucher_detail_no):
            frappe.db.set_value("Subcontracting Receipt Item",
                                sle.voucher_detail_no, "rate", outgoing_rate)
        else:
            frappe.db.set_value(
                "Subcontracting Receipt Supplied Item",
                sle.voucher_detail_no,
                {"rate": outgoing_rate, "amount": abs(
                    sle.actual_qty) * outgoing_rate},
            )

        scr = frappe.get_doc("Subcontracting Receipt",
                             sle.voucher_no, for_update=True)
    ...
    def update_rate_on_stock_reconciliation(self, sle):
        if not sle.serial_no and not sle.batch_no:
            sr = frappe.get_doc("Stock Reconciliation",
                                sle.voucher_no, for_update=True)
            ...
            for item in sr.items:
                ...
                item.current_qty = previous_sle.get(
                    "qty_after_transaction") or 0.0
                item.current_valuation_rate = previous_sle.get(
                    "valuation_rate") or 0.0
                item.current_amount = flt(
                    item.current_qty) * flt(item.current_valuation_rate)

                item.amount = flt(item.qty) * flt(item.valuation_rate)
                item.quantity_difference = item.qty - item.current_qty
                item.amount_difference = item.amount - item.current_amount
            else:
                sr.difference_amount = sum(
                    [item.amount_difference for item in sr.items])
            sr.db_update()
    ...
    def get_incoming_value_for_serial_nos(self, sle, serial_nos):
        ...
        incoming_values = sum(flt(d.purchase_rate)
                              for d in all_serial_nos if d.company == sle.company)

        # Get rate for serial nos which has been transferred to other company
        invalid_serial_nos = [
            d.name for d in all_serial_nos if d.company != sle.company]
    ...
    def update_queue_values(self, sle):
        ...
        if self.wh_data.qty_after_transaction:
            self.wh_data.valuation_rate = self.wh_data.stock_value / \
                self.wh_data.qty_after_transaction
    ...
    def get_sle_before_datetime(self, args):
        ...
        sle = get_stock_ledger_entries(
            args, "<", "desc", "limit 1", for_update=False)
    ...
    def raise_exceptions(self):
            ...
            if (
                ...
                msg = _("{0} units of {1} needed in {2} to complete this transaction.").format(
                    ...
                    frappe.get_desk_link(
                        "Item", exceptions[0]["item_code"], show_title_with_name=True),
                ...
                msg = _(
                    ...
                    frappe.get_desk_link(
                        exceptions[0]["voucher_type"], exceptions[0]["voucher_no"]),
                )

            if msg:
                if self.reserved_stock:
                    allowed_qty = abs(
                        exceptions[0]["actual_qty"]) - abs(exceptions[0]["diff"])

                    if allowed_qty > 0:
                        msg = "{} As {} units are reserved for other sales orders, you are allowed to consume only {} units.".format(
                            msg, frappe.bold(
                                self.reserved_stock), frappe.bold(allowed_qty)
                        )
                    else:
        ...
        if msg_list:
            message = "\n\n".join(msg_list)
            if self.verbose:
                frappe.throw(message, NegativeStockError,
    ...
    def update_bin(self):
        ...
        for warehouse, data in self.data.items():
            ...
            updated_values = {
                "actual_qty": data.qty_after_transaction, "stock_value": data.stock_value}
            if data.valuation_rate is not None:
                updated_values["valuation_rate"] = data.valuation_rate
            frappe.db.set_value(
                "Bin", bin_name, updated_values, update_modified=True)
...
def get_sle_by_voucher_detail_no(voucher_detail_no, excluded_sle=None):
    return frappe.db.get_value(
        "Stock Ledger Entry",
        {"voucher_detail_no": voucher_detail_no, "name": [
            "!=", excluded_sle], "is_cancelled": 0},
        [
            "item_code",
            "warehouse",
            "actual_qty",
...
def get_valuation_rate(
    ...
    if not valuation_rate:
        ...
        valuation_rate = frappe.db.get_value(
            "Item", item_code, "standard_rate")

        if not valuation_rate:
            # try in price list
            valuation_rate = frappe.db.get_value(
                "Item Price", dict(item_code=item_code, buying=1,
                                   currency=currency), "price_list_rate"
            )
    ...
    if (
        ...
        sub_solutions = "<ul><li>" + \
            _("Create an incoming stock transaction for the Item.") + "</li>"
        sub_solutions += "<li>" + \
            _("Mention Valuation Rate in the Item master.") + "</li></ul>"
...
def get_stock_reco_qty_shift(args):
    ...
    if args.get("is_cancelled"):
        if args.get("previous_qty_after_transaction"):
            ...
            stock_reco_qty_shift = flt(
                args.qty_after_transaction) - flt(last_balance)
        else:
...
def get_datetime_limit_condition(detail):
    posting_datetime = get_combine_datetime(
        detail.posting_date, detail.posting_time)
...
def validate_negative_qty_in_future_sle(args, allow_negative_stock=False):
    ...
    if is_negative_with_precision(neg_sle):
        message = _("{0} units of {1} needed in {2} on {3} {4} for {5} to complete this transaction.").format(
            ...
            frappe.get_desk_link(
                neg_sle[0]["voucher_type"], neg_sle[0]["voucher_no"]),
        )
    ...
    if args.batch_no:
        ...
        if is_negative_with_precision(neg_batch_sle, is_batch=True):
            message = _(
                ...
                frappe.get_desk_link(
                    neg_batch_sle[0]["voucher_type"], neg_batch_sle[0]["voucher_no"]),
            )
            frappe.throw(message, NegativeStockError,
                         title=_("Insufficient Stock for Batch"))
...
def validate_reserved_stock(kwargs):
    if kwargs.serial_no:
        ...
        validate_reserved_serial_nos(
            kwargs.item_code, kwargs.warehouse, serial_nos)

    elif kwargs.batch_no:
        validate_reserved_batch_nos(
            kwargs.item_code, kwargs.warehouse, [kwargs.batch_no])
        ...
        if serial_nos := [entry.serial_no for entry in sbb_entries if entry.serial_no]:
            ...
            validate_reserved_batch_nos(
                kwargs.item_code, kwargs.warehouse, batch_nos)
...
def validate_reserved_batch_nos(item_code, warehouse, batch_nos):
    if reserved_batches_map := get_sre_reserved_batch_nos_details(item_code, warehouse, batch_nos):
        ...
        available_batches_map = {
            row.batch_no: row.qty for row in available_batches}
        precision = cint(frappe.db.get_default("float_precision")) or 2

        for batch_no in batch_nos:
            diff = flt(
                available_batches_map.get(
                    batch_no, 0) - reserved_batches_map.get(batch_no, 0), precision
            )
            if diff < 0 and abs(diff) > 0.0001:
                msg = _("{0} units of {1} needed in {2} on {3} {4} to complete this transaction.").format(
                    abs(diff),
