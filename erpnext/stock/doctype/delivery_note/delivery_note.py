...
class DeliveryNote(SellingController):
    ...
    def before_print(self, settings=None):
        ...
        for key, fieldname in print_hide_fields.items():
            for f in fieldname:
                toggle_print_hide(self.meta if key ==
                                  "parent" else item_meta, f)
        ...
        for item in self.items:
            ...
            if item.pick_list_item and not item.serial_and_batch_bundle:
                ...
                bundle_id = frappe.db.get_value(
                    "Serial and Batch Bundle", filters, "name")
    ...
    def validate_sales_order_references(self):
        self._validate_dependent_item_fields(
            "against_sales_order", "so_detail", _(
                "References to Sales Orders are Incomplete")
        )

    def validate_sales_invoice_references(self):
        self._validate_dependent_item_fields(
            "against_sales_invoice", "si_detail", _(
    ...
    def update_pick_list_status(self):
        ...
        pick_lists = {
            row.against_pick_list for row in self.items if row.against_pick_list}
    ...
    def check_next_docstatus(self):
        ...
        if submit_rv:
            frappe.throw(
                _("Sales Invoice {0} has already been submitted").format(submit_rv[0][0]))
        ...
        if submit_in:
            frappe.throw(
                _("Installation Note {0} has already been submitted").format(submit_in[0][0]))
    ...
    def update_billing_status(self, update_modified=True):
        ...
        for dn in set(updated_delivery_notes):
            dn_doc = self if (dn == self.name) else frappe.get_doc(
                "Delivery Note", dn)
    ...
    def make_return_invoice(self):
        try:
            ...
            credit_note_link = frappe.utils.get_link_to_form(
                "Sales Invoice", return_invoice.name)

            frappe.msgprint(
                _("Credit Note {0} has been created automatically").format(credit_note_link))
        except Exception:
            frappe.throw(
                _(
        ...
        source_name,
        {
            ...
            "Delivery Note Item": {
                ...
                "condition": lambda item: (
                    not frappe.db.exists("Product Bundle", {
    ...
    def update_item(source, target, source_parent):
        if source_parent.doctype == "Delivery Note" and source.received_qty:
            target.qty = flt(source.qty) + \
                flt(source.returned_qty) - flt(source.received_qty)
