...
class StockEntry(StockController):
    ...
    def before_validate(self):
        ...
        if self.get("items") and apply_rule:
            apply_putaway_rule(self.doctype, self.get(
                "items"), self.company, purpose=self.purpose)
    ...
    def validate(self):
        ...
        if not self.get("purpose") == "Manufacture":
            ...
            self.reset_default_field_value(
                "from_warehouse", "items", "s_warehouse")
            self.reset_default_field_value(
                "to_warehouse", "items", "t_warehouse")
    ...
    def validate_work_order_status(self):
        ...
        if pro_doc.status == "Completed":
            frappe.throw(
                _("Cannot cancel transaction for Completed Work Order."))
    ...
    def validate_purpose(self):
        ...
        if self.purpose not in valid_purposes:
            frappe.throw(_("Purpose must be one of {0}").format(
    ...
    def set_transfer_qty(self):
        ...
        for item in self.get("items"):
            if not flt(item.conversion_factor):
                frappe.throw(
                    _("Row {0}: UOM Conversion Factor is mandatory").format(item.idx))
            item.transfer_qty = flt(
                flt(item.qty) *
                flt(item.conversion_factor), self.precision(
                    "transfer_qty", item)
            )
    ...
    def validate_item(self):
        ...
        for item in self.get("items"):
            ...
            if item.item_code not in stock_items:
                frappe.throw(
                    _("{0} is not a stock Item").format(item.item_code))
        ...
        for item_code, qty_list in item_wise_qty.items():
            ...
            if (self.fg_completed_qty - total) > 0 and not self.process_loss_qty:
                self.process_loss_qty = flt(
                    self.fg_completed_qty - total, precision)
                self.process_loss_percentage = flt(
                    self.process_loss_qty * 100 / self.fg_completed_qty)
        ...
        for d in self.get("items"):
            ...
            if self.purpose in source_mandatory and not d.s_warehouse:
                if self.from_warehouse:
                    ...
                    frappe.throw(
                        _("Source warehouse is mandatory for row {0}").format(d.idx))
            ...
            if self.purpose in target_mandatory and not d.t_warehouse:
                if self.to_warehouse:
                    ...
                    frappe.throw(
                        _("Target warehouse is mandatory for row {0}").format(d.idx))
            ...
            if cstr(d.s_warehouse) == cstr(d.t_warehouse) and self.purpose not in [
                ...
                frappe.throw(
                    _("Source and target warehouse cannot be same for row {0}").format(d.idx))
        ...
        if self.purpose in (
            ...
            if (
                ...
                if not self.fg_completed_qty:
                    frappe.throw(
                        _("For Quantity (Manufactured Qty) is mandatory"))
    ...
    def check_if_operations_completed(self):
        ...
        for d in prod_order.get("operations"):
            total_completed_qty = flt(
                self.fg_completed_qty) + flt(prod_order.produced_qty)
            completed_qty = (
                d.completed_qty + d.process_loss_qty +
                (allowance_percentage / 100 * d.completed_qty)
            )
            if flt(total_completed_qty, self.precision("fg_completed_qty")) > flt(
                ...
                job_card = frappe.db.get_value(
                    "Job Card", {"operation_id": d.name}, "name")
                if not job_card:
                    frappe.throw(
                ...
                work_order_link = get_link_to_form(
    ...
    def set_actual_qty(self):
        ...
        for d in self.get("items"):
            allow_negative_stock = is_negative_stock_allowed(
            ...
            if (
                ...
                frappe.throw(
                    ...
                    + _("Available quantity is {0}, you need {1}").format(
                        frappe.bold(flt(d.actual_qty, d.precision(
                            "actual_qty"))), frappe.bold(d.transfer_qty)
                    ),
                    NegativeStockError,
            ...
            if matched_item := self.get_matched_items(item_code):
                ...
                frappe.throw(
                    _("According to the BOM {0}, the Item '{1}' is missing in the stock entry.").format(
                        get_link_to_form(
                            "BOM", self.bom_no), frappe.bold(item_code)
                    ),
        ...
        for d in self.get("items"):
            ...
            if d.allow_zero_valuation_rate:
                ...
                if self.purpose == "Manufacture":
                    ...
                    d.basic_rate = self.get_basic_rate_for_repacked_items(
            ...
            d.basic_amount = flt(flt(d.transfer_qty) *
                                 flt(d.basic_rate), d.precision("basic_amount"))
    ...
    def set_rate_for_outgoing_items(self, reset_outgoing_rate=True, raise_error_if_no_rate=True):
        ...
        for d in self.get("items"):
            if d.s_warehouse:
                ...
                d.basic_amount = flt(
                    flt(d.transfer_qty) * flt(d.basic_rate), d.precision("basic_amount"))
                if not d.t_warehouse:
    ...
    def get_basic_rate_for_repacked_items(self, finished_item_qty, outgoing_items_cost):
        finished_items = [d.item_code for d in self.get(
            "items") if d.is_finished_item]
        if len(finished_items) == 1:
            ...
            if len(unique_finished_items) == 1:
                total_fg_qty = sum([flt(d.transfer_qty)
                                   for d in self.items if d.is_finished_item])
                return flt(outgoing_items_cost / total_fg_qty)

    def get_basic_rate_for_manufactured_item(self, finished_item_qty, outgoing_items_cost=0) -> float:
        settings = frappe.get_single("Manufacturing Settings")
        scrap_items_cost = sum([flt(d.basic_amount)
                               for d in self.get("items") if d.is_scrap_item])
        ...
        if settings.material_consumption:
            if settings.get_rm_cost_from_consumption_entry and self.work_order:
                        ...
                        if not item.is_finished_item and not item.is_scrap_item:
                            ...
                            frappe.throw(
                                _(
                                    ...
                                    frappe.bold(
                                        _("Material Consumption for Manufacture")),
                                )
                            )
                    ...
                    if frappe.db.exists(
                        ...
                        frappe.throw(
                            _("Only one {0} entry can be created against the Work Order {1}").format(
                                frappe.bold(_("Manufacture")), frappe.bold(
                                    self.work_order)
                            )
                        )
                ...
                outgoing_items_cost = sum(
                    [flt(row.qty) * flt(row.rate) for row in bom_items.values()])
    ...
    def distribute_additional_costs(self):
        ...
        self.total_additional_costs = sum(
            flt(t.base_amount) for t in self.get("additional_costs"))
        ...
        for d in self.get("items"):
            if self.purpose in ("Repack", "Manufacture") and not d.is_finished_item:
                ...
                continue
            d.additional_cost = (
                flt(d.basic_amount) / incoming_items_cost) * self.total_additional_costs

    def update_valuation_rate(self):
        for d in self.get("items"):
            if d.transfer_qty:
                d.amount = flt(flt(d.basic_amount) +
                               flt(d.additional_cost), d.precision("amount"))
                # Do not round off valuation rate to avoid precision loss
                d.valuation_rate = flt(
                    d.basic_rate) + (flt(d.additional_cost) / flt(d.transfer_qty))
    ...
    def set_total_amount(self):
        ...
        if self.purpose not in ["Manufacture", "Repack"]:
            self.total_amount = sum([flt(item.amount)
                                    for item in self.get("items")])
    ...
    def validate_subcontract_order(self):
        ...
        qty_allowance = flt(frappe.db.get_single_value(
        ...
        if backflush_raw_materials_based_on == "BOM":
            subcontract_order = frappe.get_doc(
                self.subcontract_data.order_doctype, self.get(
            ...
            for se_item in self.items:
                ...
                total_allowed = required_qty + \
                    (required_qty * (qty_allowance / 100))
                ...
                if not required_qty:
                    ...
                    if se_item.allow_alternative_item:
                        original_item_code = frappe.get_value(
                            "Item Alternative", {
                                "alternative_item_code": item_code}, "item_code"
                        )
                ...
                if flt(total_supplied - total_returned, precision) > flt(total_allowed, precision):
                    ...
                    if order_rm_detail:
                        se_item.db_set(
                            self.subcontract_data.rm_detail_field, order_rm_detail)
                    else:
    ...
    def validate_closed_subcontracting_order(self):
        if self.get("subcontracting_order"):
            check_on_hold_or_closed_status(
                "Subcontracting Order", self.subcontracting_order)

    def mark_finished_and_scrap_items(self):
        if self.purpose != "Repack" and any(
                [d.item_code for d in self.items if (
                    d.is_finished_item and d.t_warehouse)]
        ):
            return
    ...
    def get_finished_item(self):
        finished_item = None
        if self.work_order:
            finished_item = frappe.db.get_value(
    ...
    def validate_finished_goods(self):
        ...
        if self.work_order:
            wo_details = frappe.db.get_value("Work Order", self.work_order, [
                                             "production_item", "qty"])
            if wo_details:
    ...
    def update_stock_ledger(self, allow_negative_stock=False):
        ...
        for sle in sl_entries:
            child = frappe.get_value(
                "Stock Entry Detail", sle.voucher_detail_no, "item_id")
            if child:
                sle.item_id = child

        self.make_sl_entries(
            sl_entries, allow_negative_stock=allow_negative_stock)
    ...
    def validate_serial_batch_bundle_type(self, serial_and_batch_bundle):
        if (
                frappe.db.get_value("Serial and Batch Bundle",
                                    serial_and_batch_bundle, "type_of_transaction")
                != "Outward"
        ):
            frappe.throw(
                _(
        ...
        for d in self.get("items"):
            if cstr(d.s_warehouse):
                if d.serial_and_batch_bundle and self.docstatus == 1:
                    self.validate_serial_batch_bundle_type(
    ...
    def get_gl_entries(self, warehouse_account):
        ...
        if self.purpose in ("Repack", "Manufacture"):
            total_basic_amount = sum(flt(t.basic_amount)
                                     for t in self.get("items") if t.is_finished_item)
        else:
            total_basic_amount = sum(flt(t.basic_amount)
                                     for t in self.get("items") if t.t_warehouse)
        ...
        for t in self.get("additional_costs"):
            for d in self.get("items"):
                ...
                item_account_wise_additional_cost.setdefault(
                    (d.item_code, d.name), {})
    ...
    def update_disassembled_order(self, is_cancel=False):
        ...
        if self.purpose == "Disassemble" and self.fg_completed_qty:
            pro_doc = frappe.get_doc("Work Order", self.work_order)
            pro_doc.run_method("update_disassembled_qty",
                               self.fg_completed_qty, is_cancel)
    ...
    def get_item_details(self, args=None, for_update=False):
        ...
        if args.get("uom") and for_update:
            ret.update(get_uom_details(args.get("item_code"),
                       args.get("uom"), args.get("qty")))

        if self.purpose == "Material Issue":
            ret["expense_account"] = item.get(
                "expense_account") or item_group_defaults.get("expense_account")
        ...
        for company_field, field in {
            ...
            if not ret.get(field):
                ret[field] = frappe.get_cached_value(
                    "Company", self.company, company_field)

        args["posting_date"] = self.posting_date
        args["posting_time"] = self.posting_time

        stock_and_rate = get_warehouse_details(
            args) if args.get("warehouse") else {}
    ...
    def set_items_for_stock_in(self):
        ...
        if self.outgoing_stock_entry and self.purpose == "Material Transfer":
            ...
            if doc.per_transferred == 100:
                frappe.throw(
                    _("Goods are already received against the outward entry {0}").format(doc.name))
    ...
    def get_items_for_disassembly(self, disassemble_qty, production_item):
        ...
        if not self.work_order:
            frappe.throw(
                _("The Work Order is mandatory for Disassembly Order"))

        items = self.get_items_from_manufacture_entry()

        s_warehouse = frappe.db.get_value(
            "Work Order", self.work_order, "fg_warehouse")

        items_dict = get_bom_items_as_dict(
            self.bom_no, self.company, disassemble_qty)
        ...
        for row in items:
            ...
            child_row.s_warehouse = (
                self.from_warehouse or s_warehouse) if row.is_finished_item else ""
        ...
        if self.bom_no:
            ...
            if self.purpose in [
                ...
                if self.work_order and self.purpose == "Material Transfer for Manufacture":
                    ...
                    item_dict = self.get_bom_raw_materials(
                    ...
                    if (
                        ...
                        # Get Subcontract Order Supplied Items Details
                        parent = frappe.qb.DocType(
                            self.subcontract_data.order_doctype)
                        child = frappe.qb.DocType(
                            self.subcontract_data.order_supplied_items_field)
                    ...
                    for original_item, item in item_dict.items():
                        ...
                        if (
                            ...
                            item["from_warehouse"] = item_wh.get(
                                item.item_code)
                        item["to_warehouse"] = (
    ...
    def set_scrap_items(self):
        if self.purpose != "Send to Subcontractor" and self.purpose in ["Manufacture", "Repack"]:
            scrap_item_dict = self.get_bom_scrap_material(
                self.fg_completed_qty)
        ...
        if self.process_loss_percentage and not self.process_loss_qty:
            self.process_loss_qty = flt(
                (flt(self.fg_completed_qty) *
                 flt(self.process_loss_percentage)) / 100
            )
    ...
    def set_batchwise_finished_goods(self, args, item):
        batches = get_empty_batches_based_work_order(
            self.work_order, self.pro_doc.production_item)
    ...
    def get_bom_raw_materials(self, qty):
        ...
        for item in item_dict.values():
            ...
            if item.item_code in used_alternative_items:
                alternative_item_data = used_alternative_items.get(
                    item.item_code)
    ...
    def get_bom_scrap_material(self, qty):
        ...
        if (
            ...
            item_dict = get_scrap_items_from_sub_assemblies(
                self.bom_no, self.company, qty)
        else:
            # item dict = { item_code: {qty, description, stock_uom} }
            item_dict = (
                get_bom_items_as_dict(
    ...
    def get_scrap_items_from_job_card(self):
        ...
        pending_qty = flt(self.get_completed_job_card_qty()
                          ) - flt(self.pro_doc.produced_qty)

        used_scrap_items = self.get_used_scrap_items()
        for row in scrap_items:
            row.stock_qty -= flt(used_scrap_items.get(row.item_code))
            row.stock_qty = (row.stock_qty) * \
                flt(self.fg_completed_qty) / flt(pending_qty)
    ...
    def get_used_scrap_items(self):
        ...
        data = frappe.get_all(
            "Stock Entry",
            fields=["`tabStock Entry Detail`.`item_code`",
                    "`tabStock Entry Detail`.`qty`"],
            filters=[
                ["Stock Entry", "work_order", "=", self.work_order],
                ["Stock Entry Detail", "is_scrap_item", "=", 1],
                ["Stock Entry", "docstatus", "=", 1],
                ["Stock Entry", "purpose", "in",
                 ["Repack", "Manufacture"]],
            ],
        )
    ...
    def get_unconsumed_raw_materials(self):
        ...
        wo_items = frappe.get_all(
            ...
            fields=["item_code", "source_warehouse",
                    "required_qty", "consumed_qty", "transferred_qty"],
        )

        work_order_qty = wo.material_transferred_for_manufacturing or wo.qty
        for item in wo_items:
            item_account_details = get_item_defaults(
                item.item_code, self.company)
    ...
    def add_transfered_raw_materials_in_items(self) -> None:
        ...
        wo_data = frappe.db.get_value(
            ...
            self.work_order,
            ["qty", "produced_qty",
                    "material_transferred_for_manufacturing as trans_qty"],
            as_dict=1,
        )

        precision = frappe.get_precision("Stock Entry Detail", "qty")
        for _key, row in available_materials.items():
            remaining_qty_to_produce = flt(
                wo_data.trans_qty) - flt(wo_data.produced_qty)
    ...
    def update_batches_to_be_consume(self, batches, row, qty):
        ...
        for batch_no, batch_qty in batches:
            ...
            if batch_no and row.serial_nos:
                serial_nos = self.get_serial_nos_based_on_transferred_batch(
                    batch_no, row.serial_nos)
    ...
    def update_item_in_stock_entry_detail(self, row, item, qty) -> None:
        ...
        if use_serial_batch_fields and not row.serial_no and row.batches_to_be_consume:
            for batch_no, batch_qty in row.batches_to_be_consume.items():
                ...
                self.add_to_stock_entry_detail(
                    {item.item_code: ste_item_details})
        else:
    ...
    def get_serial_nos_based_on_transferred_batch(batch_no, serial_nos) -> list:
        serial_nos = frappe.get_all(
            "Serial No",
            filters={"batch_no": batch_no, "name": (
                "in", serial_nos), "warehouse": ("is", "not set")},
            order_by="creation",
        )
    ...
    def get_pending_raw_materials(self, backflush_based_on=None):
        ...
        transfer_limit_qty = max_qty + \
            ((max_qty * overproduction_percentage) / 100)

        if transfer_limit_qty >= to_transfer_qty:
            allow_overproduction = True

        for item, item_details in item_dict.items():
            pending_to_issue = flt(item_details.required_qty) - \
                flt(item_details.transferred_qty)
            desire_to_transfer = flt(
                self.fg_completed_qty) * flt(item_details.required_qty) / max_qty
            ...
            if (
                ...
                item_dict[item]["qty"] = desire_to_transfer if (
                    desire_to_transfer > 0) else pending_to_issue
        ...
        if not len(item_dict):
            frappe.msgprint(
                _("""All items have already been transferred for this Work Order."""))
    ...
    def get_pro_order_required_items(self, backflush_based_on=None):
        ...
        consider_job_card = work_order.transfer_material_against == "Job Card" and self.get(
            "job_card")
            ...
            if d.include_item_in_manufacturing:
                ...
                if consider_job_card:
                    job_card_item = frappe.db.get_value(
                        "Job Card Item", {
                            "item_code": d.item_code, "parent": self.get("job_card")}
                    )
    ...
    def add_to_stock_entry_detail(self, item_dict, bom_no=None):
        ...
        for d in item_dict:
            ...
            se_child.uom = item_row["uom"] if item_row.get(
                "uom") else stock_uom
            ...
            # in stock uom
            se_child.conversion_factor = flt(
                item_row.get("conversion_factor")) or 1
            se_child.transfer_qty = flt(
                item_row["qty"] *
                se_child.conversion_factor, se_child.precision("qty")
            )

            se_child.bom_no = bom_no  # to be assigned for finished item
            se_child.job_card_item = item_row.get(
                "job_card_item") if self.get("job_card") else None
    ...
    def update_subcontract_order_supplied_items(self):
        if self.get(self.subcontract_data.order_field) and (
                self.purpose in ["Send to Subcontractor",
                                 "Material Transfer"] or self.is_return
        ):
            # Get Subcontract Order Supplied Items Details
            order_supplied_items = frappe.db.get_all(
                self.subcontract_data.order_supplied_items_field,
                filters={"parent": self.get(
                    self.subcontract_data.order_field)},
            ...
            for row in order_supplied_items:
                ...
                if not supplied_items.get(key):
                    ...
                    item = {"supplied_qty": 0, "returned_qty": 0,
                            "total_supplied_qty": 0}
                else:
                    item = supplied_items.get(key)

                frappe.db.set_value(
                    self.subcontract_data.order_supplied_items_field, row.name, item)

            # RM Item-Reserve Warehouse Dict
            item_wh = {x.get("rm_item_code"): x.get("reserve_warehouse")
                       for x in order_supplied_items}
            ...
            if stock_entries_child_list:
                frappe.db.sql(
                    """ UPDATE `tabStock Entry Detail`
                        ...
                        cond=cond, ste_details=",".join(
                            ["%s"] * len(stock_entries_child_list))
                    ),
    ...
    def set_material_request_transfer_status(self, status):
        ...
        if self.outgoing_stock_entry:
            parent_se = frappe.get_value(
                "Stock Entry", self.outgoing_stock_entry, "add_to_transit")
        ...
        for item in self.items:
            ...
            if material_request and material_request not in material_requests:
                ...
                frappe.db.set_value(
                    "Material Request", material_request, "transfer_status", status)
    ...
    def set_serial_no_batch_for_finished_good(self):
        ...
        for d in self.items:
            if (
                ...
                if serial_nos:
                    row = frappe._dict(
                        {"serial_nos": serial_nos[0: cint(d.qty)]})
    ...
    if work_order:
        ...
        for d in work_order.get("operations"):
            if flt(d.completed_qty):
                operating_cost_per_unit += flt(d.actual_operating_cost) / \
                    flt(d.completed_qty)
            elif work_order.qty:
                operating_cost_per_unit += flt(
                    d.planned_operating_cost) / flt(work_order.qty)

    # Get operating cost from BOM if not found in work_order.
    if not operating_cost_per_unit and bom_no:
        bom = frappe.db.get_value(
            "BOM", bom_no, ["operating_cost", "quantity"], as_dict=1)
        if bom.quantity:
            operating_cost_per_unit = flt(
...
def get_valuation_rate_for_finished_good_entry(work_order):
    work_order_qty = flt(
        frappe.get_cached_value("Work Order", work_order,
...
def get_uom_details(item_code, uom, qty):
    ...
    conversion_factor = get_conversion_factor(
        item_code, uom).get("conversion_factor")
...
def validate_sample_quantity(item_code, sample_quantity, qty, batch_no=None):
    if cint(qty) < cint(sample_quantity):
        frappe.throw(
            _("Sample quantity {0} cannot be more than received quantity {1}").format(
                sample_quantity, qty)
        )
    retention_warehouse = frappe.db.get_single_value(
        "Stock Settings", "sample_retention_warehouse")
    ...
    for row in frappe.get_all("Stock Entry", fields=fields, filters=filters):
        ...
        if key not in supplied_item_details:
            supplied_item_details.setdefault(
                key, frappe._dict(
                    {"supplied_qty": 0, "returned_qty": 0, "total_supplied_qty": 0})
            )
        ...
        supplied_item.total_supplied_qty = flt(
            supplied_item.supplied_qty) - flt(supplied_item.returned_qty)
    ...
    for row in data:
        ...
        if key not in available_materials:
            available_materials.setdefault(
                key,
                frappe._dict(
                    {"item_details": row, "batch_details": defaultdict(
                        float), "qty": 0, "serial_nos": []}
                ),
            )
...
def get_stock_entry_data(work_order):
    ...
    if voucher_nos:
        bundle_data = get_voucher_wise_serial_batch_from_bundle(
...
def create_serial_and_batch_bundle(parent_doc, row, child, type_of_transaction=None):
    ...
    if row.serial_nos and row.batches_to_be_consume:
        ...
        for serial_no in row.serial_nos:
            doc.append("entries", {"serial_no": serial_no,
                       "warehouse": row.warehouse, "qty": -1})
        ...
        for batch_no, qty in row.batches_to_be_consume.items():
            if flt(qty, precision) > 0:
                ...
                doc.append(
                    "entries", {"batch_no": batch_no, "warehouse": row.warehouse, "qty": qty * -1})
...
def get_batchwise_serial_nos(item_code, row):
    ...
    for batch_no in row.batches_to_be_consume:
        serial_nos = frappe.get_all(
            "Serial No",
            filters={"item_code": item_code, "batch_no": batch_no,
                     "name": ("in", row.serial_nos)},
        )

        if serial_nos:
            batchwise_serial_nos[batch_no] = sorted(
                [serial_no.name for serial_no in serial_nos])
