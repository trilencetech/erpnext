...
class PurchaseReceipt(BuyingController):
    ...
    def before_submit(self):
        ...
        for item in self.items:
            ...
            if roll_weights:
                weights = [float(w.strip())
                           for w in item.roll_weights.split(",") if w.strip()]
                total_weight = sum(weights)
                if total_weight != item.qty:
                    frappe.throw(
                        f"Roll weights total ({total_weight}) does not match item qty ({item.qty})")
                for rw in weights:
    ...
    def validate_uom_is_integer(self):
        super().validate_uom_is_integer(
            "uom", ["qty", "received_qty"], "Purchase Receipt Item")
        ...
        if frappe.db.get_value("Buying Settings", None, "po_required") == "Yes":
            for d in self.get("items"):
                if not d.purchase_order:
                    frappe.throw(
                        _("Purchase Order number required for Item {0}").format(d.item_code))
    ...
    def check_next_docstatus(self):
        ...
        if submit_rv:
            frappe.throw(_("Purchase Invoice {0} is already submitted").format(
                self.submit_rv[0][0]))
    ...
    def on_cancel(self):
        ...
        if submitted:
            frappe.throw(
                _("Purchase Invoice {0} is already submitted").format(submitted[0][0]))
    ...
    def get_gl_entries(self, warehouse_account=None, via_landed_cost_voucher=False):
        ...
        self.make_item_gl_entries(
            gl_entries, warehouse_account=warehouse_account)
    ...
    def make_item_gl_entries(self, gl_entries, warehouse_account=None):
        ...
        def validate_account(account_type):
            frappe.throw(
                _("{0} account not found while submitting purchase receipt").format(account_type))
        ...
        def make_stock_received_but_not_billed_entry(item):
            ...
            if self.is_internal_transfer() and item.valuation_rate:
                outgoing_amount = abs(get_stock_value_difference(
            ...
            if item.get("rejected_qty") and frappe.db.get_single_value(
                ...
                outgoing_amount += get_stock_value_difference(
                    self.name, item.name, item.rejected_warehouse)
                credit_amount = outgoing_amount
        ...
        def make_divisional_loss_gl_entry(item, outgoing_amount):
            ...
            divisional_loss = flt(
                valuation_amount_as_per_doc -
                flt(stock_value_diff), item.precision("base_net_amount")
            )

            if item.get("rejected_qty") and frappe.db.get_single_value(
                    "Buying Settings", "set_valuation_rate_for_rejected_materials"
            ):
                rejected_item_cost = get_stock_value_difference(
                    self.name, item.name, item.rejected_warehouse)
        ...
        for d in self.get("items"):
            if (
                ...
                self.add_provisional_gl_entry(
                    d, gl_entries, self.posting_date, d.get(
                ...
                if not (
                        (erpnext.is_perpetual_inventory_enabled(
                            self.company) and d.item_code in stock_items)
                ...
                if d.is_fixed_asset:
                    ...
                    stock_value_diff = (
                        flt(d.base_net_amount) + flt(d.item_tax_amount) +
                ...
                warehouse_with_no_account.append(
                    d.warehouse or d.rejected_warehouse)
            ...
            if d.rejected_qty and frappe.db.get_single_value(
                ...
                stock_value_diff = get_stock_value_difference(
                    self.name, d.name, d.rejected_warehouse)
    ...
    def add_provisional_gl_entry(
        ...
        if reverse:
            ...
            expense_account = frappe.db.get_value(
                "Purchase Receipt Item", {"name": item.get("pr_detail")}, [
                    "expense_account"]
            )
    ...
    def make_tax_gl_entries(self, gl_entries, via_landed_cost_voucher=False):
        negative_expense_to_be_booked = sum(
            [flt(d.item_tax_amount) for d in self.get("items")])
    ...
    def update_billing_status(self, update_modified=True):
        ...
        if po_details:
            updated_pr += update_billed_amount_based_on_po(
                po_details, update_modified, self)

        for pr in set(updated_pr):
            pr_doc = self if (pr == self.name) else frappe.get_doc(
                "Purchase Receipt", pr)
    ...
    def reserve_stock_for_sales_order(self):
        ...
        for item in self.items:
            if item.sales_order and item.sales_order_item:
                ...
                so_items_details_map.setdefault(
                    item.sales_order, []).append(item_details)
...
def update_billed_amount_based_on_po(po_details, update_modified=True, pr_doc=None):
    ...
    for pr_item in pr_details:
        billed_against_po = flt(
            po_billed_amt_details.get(pr_item.purchase_order_item))

        # Get billed amount directly against Purchase Receipt
        billed_amt_agianst_pr = flt(
            pr_items_billed_amount.get(pr_item.name, 0))
        ...
        if pr_item.billed_amt != billed_amt_agianst_pr:
            ...
            if pr_doc and pr_item.parent == pr_doc.name:
                pr_item = next(
                    (item for item in pr_doc.items if item.name == pr_item.name), None)
                pr_item.db_set("billed_amt", billed_amt_agianst_pr,
                               update_modified=update_modified)
    ...
    for item in pr_doc.items:
        ...
        if adjust_incoming_rate:
            ...
            if (
                ...
                adjusted_amt = (
                    flt(item.billed_amt /
                        item_wise_billed_qty.get(item.name)) - flt(item.rate)
                ) * item.qty

            adjusted_amt = flt(
                adjusted_amt * flt(pr_doc.conversion_rate), item.precision("amount"))
            item.db_set("amount_difference_with_purchase_invoice",
                        adjusted_amt, update_modified=False)
            ...
            if per_over_billed > over_billing_allowance:
                frappe.throw(
                    _("Over Billing Allowance exceeded for Purchase Receipt Item {0} ({1}) by {2}%").format(
                        item.name, frappe.bold(
                            item.item_code), per_over_billed - over_billing_allowance
                    )
                )

    percent_billed = round(
        100 * (total_billed_amount / (total_amount or 1)), 6)
    ...
    return frappe._dict(
        frappe.get_all(
            ...
            filters=[
                ...
                ["Purchase Receipt Item",
                 "purchase_receipt_item", "in", items],
            ],
...
def make_purchase_invoice(source_name, target_doc=None, args=None):
    ...
    def update_item(source_doc, target_doc, source_parent):
        ...
        target_doc.stock_qty = flt(target_doc.qty) * flt(
            target_doc.conversion_factor, target_doc.precision(
                "conversion_factor")
        )
...
def get_item_account_wise_additional_cost(purchase_document):
    ...
    for lcv in landed_cost_vouchers:
        landed_cost_voucher_doc = frappe.get_doc(
            "Landed Cost Voucher", lcv.parent)

        based_on_field = None
        # Use amount field for total item cost for manually cost distributed LCVs
        if landed_cost_voucher_doc.distribute_charges_based_on != "Distribute Manually":
            based_on_field = frappe.scrub(
                landed_cost_voucher_doc.distribute_charges_based_on)
        ...
        for item in landed_cost_voucher_doc.items:
            if item.receipt_document == purchase_document:
                for account in landed_cost_voucher_doc.taxes:
                    ...
                    item_account_wise_cost.setdefault(
                        (item.item_code, item.purchase_receipt_item), {})
                    ...
                    if total_item_cost > 0:
                        item_row["amount"] += account.amount * \
                            item.get(based_on_field) / total_item_cost
                        ...
                        item_row["amount"] += item.applicable_charges / \
                            exchange_rate
...
def make_lcv(doctype, docname):
    ...
    details = frappe.db.get_value(
        doctype, docname, ["supplier", "company", "base_grand_total"], as_dict=1)
...
def get_size_inch(width_mm):
    ...
    # Convert to string and strip trailing .0
    size_str = str(size)
...
@frappe.whitelist()
def import_items_from_excel1(docname, file_url):
    doc = frappe.get_doc("Purchase Receipt", docname)

    # Get File doc from file_url
    file_doc = frappe.get_doc("File", {"file_url": file_url})

    # Read Excel content
    content = file_doc.get_content()
    rows = read_xlsx_file_from_attached_file(None, content, None)
    ...
    for row in rows[1:]:
        ...
        prefix = frappe.db.get_value(
            "Master Item Code From Thickness",
            {"thick": thick, "film_type": film_type},
            "prefix"
        )
        if not prefix:
            frappe.msgprint(
                f"No prefix found for Thick={thick}, FilmType={film_type}")
            continue

        # Convert width to inches and round to 1 decimal
        size_inch = get_size_inch(width_mm)
        item_code = f"{prefix} {size_inch}"
...
@frappe.whitelist()
def import_items_from_excel(docname, file_url):
    doc = frappe.get_doc("Purchase Receipt", docname)
    ...
    for sheet_name in wb.sheetnames:
        ...
        for row in ws.iter_rows(min_row=2, values_only=True):
            ...
            # Convert width to inches and normalize
            width_inch = round(float(width_mm) / 25.4, 1)
            size_str = str(width_inch)
            if size_str.endswith(".0"):
                size_str = size_str[:-2]

            # Build item code as "PREFIX SIZE"
            item_code = f"{prefix} {size_str}"
